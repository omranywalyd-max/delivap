...
const defaultOrigins = ['https://api.delivap.com', 'https://delivap.com'];
...
// ── Config (public for app version check) ──
safeRoute('./routes/config', '/api');
// ── Contact (public for sending messages) ──
...
// ── Auth middleware لجميع عمليات الكتابة + قراءة الطلبيات + بيانات المستخدمين ──
const authMiddleware = require('./middleware/auth');

// Endpoints حساسة تتطلب Auth حتى للقراءة (GET)
...
app.use('/api', (req, res, next) => {
  ...
  const isProtectedRead = req.method === 'GET' && protectedReadPaths.some(p => req.path === p || req.path.startsWith(p + '/'));

  if ((isWrite || isReadOrders || isProtectedRead) && !publicPaths.includes(req.path) && !req.path.startsWith('/admin/')) {
