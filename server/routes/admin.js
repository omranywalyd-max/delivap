...
router.put('/users/:uid/toggle-phone', async (req, res) => {
  try {
    ...
    await Order.updateMany(
      ...
      { $set: { userPhoneHidden: user.phoneHidden, phoneHidden: user.phoneHidden } }
    );
    res.json(user);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.put('/stores/:id', async (req, res) => {
  try {
    const old = await Store.findById(req.params.id);
    const store = await Store.findByIdAndUpdate(req.params.id, req.body, { returnDocument: 'after' });
    if (old && old.image && store && old.image !== store.image) {
...
router.put('/drivers/:id', async (req, res) => {
  try {
    ...
    // 🔒 منع منح صلاحية التسعير لأكثر من سائق لنفس نوع المركبة في نفس المدينة
    // كامل السائقين من نفس النوع والمدينة ياخدو بهاديك التسعيرة
