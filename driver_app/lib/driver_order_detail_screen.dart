...
import 'two_point_route_card.dart';
...
bool _isAllItemsHandled(List items) {
  ...
  bool _isAllUnavailable(List items) {
  ...
  Future<void> _cancelAllUnavailable(Map<String, dynamic> data) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Text('إلغاء الطلبية', style: TextStyle(fontFamily: 'Amiri', fontWeight: FontWeight.bold)),
        content: const Text('جميع المنتجات غير متوفرة. هل تريد إلغاء الطلبية بالكامل؟', style: TextStyle(fontFamily: 'Amiri')),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('لا', style: TextStyle(fontFamily: 'Amiri'))),
          TextButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('نعم، إلغاء', style: TextStyle(fontFamily: 'Amiri', color: Colors.red))),
        ],
      ),
    );
    ...
    ApiClient.put('$_apiPrefix/${widget.doc['_id']}', {
      'status': 'cancelled',
      'cancelledBy': DriverService.uid,
      'cancelledByName': '${data['driverName'] ?? ''}'.trim(),
      'updatedAt': DateTime.now().toIso8601String(),
    });
    ...
    if (userId != null) {
      FCMHelper.sendToUser(userId: userId,
        ...
        body: 'جميع المنتجات غير متوفرة، تم إلغاء الطلبية.',
        data: {'orderId': widget.doc['_id'], 'type': 'order_cancelled'},
      );
    }
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('تم إلغاء الطلبية', style: TextStyle(fontFamily: 'Amiri')), backgroundColor: Colors.red),
      );
    }
  }
...
Future<void> _finishOrder(Map<String, dynamic> data) async {
  ...
  void _openCustomerMap(double? lat, double? lng) async {
    ...
    final url = 'https://www.google.com/maps/dir/?api=1&destination=$lat,$lng&travelmode=two_wheeler';
  ...
  Widget _buildProjectBody(Map<String, dynamic> data) {
    ...
    final status = data['status'] as String? ?? 'accepted';
    final ownerName = storeName.isNotEmpty ? storeName : 'صاحب المشروع';
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          _buildCustomerCard(data, ownerName, '', false, phoneHidden: true, label: 'صاحبة المشروع', showBadge: false),
          const SizedBox(height: 12),
              ...
              child: ClipRRect(
                ...
                child: CachedNetworkImage(
                  ...
                  errorWidget: (_, __, ___) => const Center(
                    child: Icon(CupertinoIcons.photo_fill, color: Colors.grey, size: 40),
                  ),
                ),
              ),
            ),
  ...
  Widget _buildTransportActionButtons(
    ...
    if (status == 'accepted') {
      return _primaryGradientButton(
        label: _isTransport ? 'تم رفع الزبون' : 'تم استلام الطلبية',
        icon: _isTransport ? Icons.directions_walk : Icons.inventory_2,
  ...
  Widget _buildCustomerCard(
      Map<String, dynamic> data, String name, String phone, bool verified,
      {bool phoneHidden = false, String label = 'الزبون', bool showBadge = true}) {
...
if (locationImg.isNotEmpty)
  ...
  Widget _buildSummaryCard(double subtotal, double deliveryFee) {
    ...
    final List<Map<String, dynamic>> purchasedItems = items.where((i) {
      final ps = i['purchaseStatus'] as String? ?? '';
      ...
      child: Column(
        children: [
          ...
          ...purchasedItems.map((item) {
            final itemName = (item['name'] as String?) ?? 'منتج';
            ...
            return Padding(
              ...
              child: Row(
                ...
                children: [
                  Text('${lineTotal.toInt()} DA',
                      style: const TextStyle(
                          fontSize: 12, fontWeight: FontWeight.w600,
                          color: kTextDark, fontFamily: 'Amiri')),
                  Flexible(
                    child: Text(
                        '$itemName × $qty',
                        style: const TextStyle(
                            fontSize: 11, color: kTextMid, fontFamily: 'Amiri'),
                        textAlign: TextAlign.right),
                  ),
                ],
              ),
  ...
  if (status == 'accepted' && allHandled) {
    final allUnavailable = _isAllUnavailable(items);
    if (allUnavailable) {
      return _redGradientButton(
        label: 'إلغاء الطلبية',
        icon: CupertinoIcons.xmark_circle_fill,
        onTap: () => _cancelAllUnavailable(data),
      );
    }
  ...
  Map<String, dynamic> _getStatusInfo(String status) {
    switch (status) {
      ...
      case 'cancelled':
        return {'color': kDanger, 'label': 'ملغاة', 'icon': CupertinoIcons.xmark_circle_fill};
      default:
...
class _DetailProductTileState extends State<_DetailProductTile> {
  ...
  void _showDialogImageFullscreen(String url) {
    showDialog(
      context: context,
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                if (image.isNotEmpty) ...[
                  GestureDetector(
                    onTap: () => _showDialogImageFullscreen(image),
                    child: ClipRRect(
                      borderRadius: BorderRadius.circular(12),
                      child: CachedNetworkImage(
                        imageUrl: image,
