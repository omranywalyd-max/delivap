...
import 'package:dashbord/store_owner_edit_screen.dart';
...
class _AdminDashboardMainState extends State<AdminDashboardMain> {
  ...
  Widget _updateVersionOption(BuildContext ctx) {
    return ListTile(
      leading: CircleAvatar(
        backgroundColor: Colors.green.withOpacity(0.15),
        child: const Icon(Icons.system_update_alt, color: Colors.green),
      ),
      title: const Text('تحديث جديد', style: TextStyle(fontFamily: 'Amiri', fontWeight: FontWeight.bold, color: Colors.green)),
      subtitle: const Text('فرض نسخة جديدة على التطبيقات', style: TextStyle(fontFamily: 'Amiri', fontSize: 12)),
      onTap: () {
        Navigator.pop(ctx);
        _showChooseAppForUpdate();
      },
    );
  }

  void _showChooseAppForUpdate() {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: _kBg,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: const Text('تحديث جديد', textAlign: TextAlign.center,
          style: TextStyle(fontFamily: 'Amiri', fontWeight: FontWeight.bold)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            _appChoiceTile(ctx, 'customer', 'تطبيق الزبون', Icons.phone_android, Colors.green),
            const SizedBox(height: 8),
            _appChoiceTile(ctx, 'driver', 'تطبيق السائق', Icons.delivery_dining, Colors.blue),
          ],
        ),
      ),
    );
  ...
  Widget _appChoiceTile(BuildContext ctx, String appKey, String appName, IconData icon, Color color) {
    return ListTile(
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
      tileColor: color.withOpacity(0.08),
      leading: CircleAvatar(backgroundColor: color, child: Icon(icon, color: Colors.white)),
      title: Text(appName, style: const TextStyle(fontFamily: 'Amiri', fontWeight: FontWeight.bold)),
      onTap: () {
        Navigator.pop(ctx);
        _showEditVersion(appKey, appName);
      },
    );
  }

  Future<void> _showEditVersion(String appKey, String appName) async {
    Map<String, dynamic> current = {};
    try {
      final data = await ApiClient.get('/api/app/config');
      final a = data[appKey];
      if (a is Map<String, dynamic>) current = a;
    } catch (_) {}
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        backgroundColor: _kBg,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: Text('تحديث $appName', textAlign: TextAlign.center,
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: 'رقم النسخة المرفوعة (مثال: 1.0.0+21)',
                hintText: '1.0.0+21',
                border: OutlineInputBorder(),
              ),
            ),
            const SizedBox(height: 12),
            const Text('بادل الرقم باش النسخة اللي مرفوعة. أي نسخة أقل من هذا الرقم سيُمنع منها التطبيق حتى يحدث.',
              textAlign: TextAlign.center, style: TextStyle(fontFamily: 'Amiri', fontSize: 12, color: Colors.grey)),
          ],
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              final raw = versionController.text.trim();
              final plusIdx = raw.lastIndexOf('+');
              String latestVersion = currentVersion;
              int minBuild = 0;
              if (plusIdx >= 0) {
                latestVersion = raw.substring(0, plusIdx).trim();
                minBuild = int.tryParse(raw.substring(plusIdx + 1).trim()) ?? 0;
              } else {
                minBuild = int.tryParse(raw) ?? 0;
              }
              final versionLabel = latestVersion.isNotEmpty ? '$latestVersion+$minBuild' : '$minBuild';
              Navigator.pop(ctx);
              final ok = await _confirmUpdateVersion(appName, minBuild, versionLabel);
              if (ok != true) return;
              try {
                await ApiClient.put('/api/app/config', {
                  'app': appKey,
                  'minBuild': minBuild,
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text('تم تفعيل النسخة الجديدة: $appName ($versionLabel)',
                      style: const TextStyle(fontFamily: 'Amiri')),
                    backgroundColor: Colors.green,
                  ));
                }
                ...
                if (mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text('فشل التحديث: $e', style: const TextStyle(fontFamily: 'Amiri')),
                    backgroundColor: Colors.red,
                  ));
                }
              }
  ...
  Future<bool?> _confirmUpdateVersion(String appName, int minBuild, String versionLabel) {
    return showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        backgroundColor: _kBg,
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: const Text('تأكيد التحديث', textAlign: TextAlign.center,
          style: TextStyle(fontFamily: 'Amiri', fontWeight: FontWeight.bold)),
        content: Text(
          'هل أنت متأكد؟\n$appName سيُطلب منه نسخة $versionLabel فأكثر.\nأي نسخة أقل سيُمنع منها التطبيق حتى يحدث.',
          textAlign: TextAlign.center,
          style: const TextStyle(fontFamily: 'Amiri'),
        ),
        actions: [
          ...
          ElevatedButton(
            ...
            child: const Text('نعم، فعّل', style: TextStyle(color: Colors.white, fontFamily: 'Amiri')),
          ),
        ],
      ),
    );
...
class _DriversActivationTabState extends State<_DriversActivationTab> {
  ...
  static const List<Map<String, String>> _vehicleCategories = [
    ...
    {'key': 'car', 'label': 'سيارة'},
    {'key': 'harbin', 'label': 'هاربين'},
    {'key': 'fourgon', 'label': 'فورغون'},
    {'key': 'minibus', 'label': 'مينيبوس'},
  ...
  Future<void> _loadDrivers() async {
    ...
    try {
      ...
      if (_selectedVehicle.isNotEmpty) {
        list.retainWhere((d) =>
            ((d['vehicleType'] as String? ?? '').replaceAll(' ', '_') ==
                _selectedVehicle));
      }
      setState(() {
        _drivers = list;
  ...
  void _selectVehicle(String key) {
    setState(() => _selectedVehicle = key);
    _loadDrivers();
  }

  // شريط تصفية السائقين حسب نوع المركبة
  Widget _vehicleFilterBar() {
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          children: _vehicleCategories.map((cat) {
            ...
            final bool active = _selectedVehicle == key;
            return Padding(
              padding: const EdgeInsets.only(right: 8),
              child: GestureDetector(
                onTap: () => _selectVehicle(key),
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: active ? _kPrimary : _kPrimary.withOpacity(0.25),
                    ),
                    boxShadow: active
                        ? null
                        : _neu(blur: 6, offset: 3),
                  ),
                  child: Text(
                    cat['label']!,
                    style: TextStyle(
  ...
  Widget _driverCard(Map<String, dynamic> d) {
    ...
    final bool isPricingEligible = isMotorcycle ||
        const ['car', 'harbin', 'fourgon', 'minibus', 'truck'].contains(vehicleType);
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              Row(
                children: [
                  // زر منح التسعيرة - للسائقين المؤهلين (دراجة/سيارة/هاربين/فورغو)
                  if (isPricingEligible)
                    Expanded(
                      child: GestureDetector(
                      onTap: () {
                  ...
                  if (isPricingEligible) const SizedBox(width: 10),
                  // زر التفعيل / الإلغاء
                  Expanded(
                    child: GestureDetector(
                      onTap: () {
...
class _OwnerDashboardState extends State<OwnerDashboard> {
  ...
  int _projectOrdersBadge = 0;

  @override
  void initState() {
    super.initState();
    _loadData();
    _loadProjectOrdersBadge();
    _initFcm();
    ...
    if (ownerId != null) {
      ...
      SocketClient().on('project:created', _onOwnerOrderCreated);
      SocketClient().on('project:updated', _onOwnerOrderCreated);
    }
  }
  ...
  Future<void> _loadProjectOrdersBadge() async {
    try {
      final storeId = widget.ownerData['magasinId'] ?? '';
      if (storeId.isEmpty) return;
      final projects =
          await ApiClient.getList('/api/projects?storeId=$storeId');
      final activeCount = projects.where((p) {
        final s = p['status'] as String? ?? '';
        return s == 'pending' || s == 'processing';
      }).length;
      if (mounted) setState(() => _projectOrdersBadge = activeCount);
    } catch (_) {}
  }
  ...
  void dispose() {
    ...
    if (ownerId != null) {
      ...
      SocketClient().off('project:created', _onOwnerOrderCreated);
      SocketClient().off('project:updated', _onOwnerOrderCreated);
  ...
  void _doImmediateLogout(BuildContext context) async {
    ApiClient.setToken(null);
    await FirebaseAuth.instance.signOut();
    final prefs = await SharedPreferences.getInstance();
    await prefs.clear();
  ...
  void _confirmLogout(BuildContext context) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => StoreOwnerEditScreen(ownerData: widget.ownerData),
      ),
    );
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'خروج (اضغط) / تعديل الحساب (اضغط مطولاً)',
            onPressed: () => _doImmediateLogout(context),
              ...
              children: [
                ...
                if ((_storeData['uiStyle'] ?? 1) == 6) ...[
                  _actionBtn(
                    ...
                    badgeCount: _projectOrdersBadge,
                  ),
...
class _CategoryEditorSheetState extends State<CategoryEditorSheet> {
  ...
  void initState() {
    ...
    if (widget.doc != null) {
      ...
      _existingImg = d['image'] ?? '';
      _openTime = d['openTime'] ?? '';
      _closeTime = d['closeTime'] ?? '';
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final data = {
        ...
        if (_address.isNotEmpty && _address != "تحديد الموقع من الخريطة") 'address': _address,
      };
                  ...
                  child: GestureDetector(
                    onTap: () async {
                      final t = await showTimePicker(
                        context: context,
                        initialTime: _openTime.isNotEmpty
                            ? TimeOfDay(
                      ...
                      if (t != null) setState(() => _openTime = '${t.hour.toString().padLeft(2, '0')}:${t.minute.toString().padLeft(2, '0')}');
                    },
                    child: _localNeuBox(
                      ...
                      child: Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          const Icon(CupertinoIcons.clock, color: _kPrimary, size: 18),
                          const SizedBox(width: 6),
                          Column(
                            children: [
                              const Text('من', style: TextStyle(fontFamily: 'Amiri', fontSize: 11, color: Colors.grey)),
                              const SizedBox(height: 2),
                              Text(
                                _openTime.isEmpty ? '00:00' : _openTime,
                                style: TextStyle(
                                  fontFamily: 'Amiri',
                                  fontSize: 15,
                                  fontWeight: FontWeight.bold,
                                  color: _openTime.isEmpty ? Colors.grey : _kPrimary,
                                ),
                              ),
                            ],
                          ),
                ...
                Expanded(
                  child: GestureDetector(
                    onTap: () async {
                      final t = await showTimePicker(
                        context: context,
                        initialTime: _closeTime.isNotEmpty
                            ? TimeOfDay(
                      ...
                      if (t != null) setState(() => _closeTime = '${t.hour.toString().padLeft(2, '0')}:${t.minute.toString().padLeft(2, '0')}');
                    },
                    child: _localNeuBox(
                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12),
                      child: Row(
                        ...
                        children: [
                          ...
                          Column(
                            children: [
                              const Text('إلى', style: TextStyle(fontFamily: 'Amiri', fontSize: 11, color: Colors.grey)),
                              const SizedBox(height: 2),
                              Text(
                                _closeTime.isEmpty ? '00:00' : _closeTime,
                                style: TextStyle(
                                  fontFamily: 'Amiri',
                                  fontSize: 15,
                                  fontWeight: FontWeight.bold,
                                  color: _closeTime.isEmpty ? Colors.grey : _kPrimary,
                                ),
                              ),
                            ],
                          ),
...
// ══════════════════════════════════════════════════════════════════════════════
//  _LogoutCountdownDialog — عداد تنازلي 4 ثواني
// ══════════════════════════════════════════════════════════════════════════════
class _LogoutCountdownDialog extends StatefulWidget {
  const _LogoutCountdownDialog();
  @override
  State<_LogoutCountdownDialog> createState() => _LogoutCountdownDialogState();
}

class _LogoutCountdownDialogState extends State<_LogoutCountdownDialog> {
  int _seconds = 4;
  Timer? _timer;
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text('جاري فتح صفحة تعديل الحساب...', style: TextStyle(fontFamily: 'Amiri', fontSize: 13, color: Colors.grey.shade600)),
          const SizedBox(height: 8),
          Text('$_seconds', style: const TextStyle(fontFamily: 'Amiri', fontSize: 28, fontWeight: FontWeight.bold, color: _kPrimary)),
        ],
      ),
    );
  }
