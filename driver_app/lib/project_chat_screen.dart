...
const Color _kChatPrimary = Color(0xFF5B0094);
const Color _kChatPrimaryLight = Color(0xFF9B59C8);
const Color _kChatBg = Color(0xFFE8E6F0);
const Color _kChatText = Color(0xFF2D2540);
const Color _kChatSub = Color(0xFF7B6E99);

/// محادثة صاحب المشروع مع الزبون
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        leading: IconButton(
          icon: const Icon(CupertinoIcons.back, color: _kChatPrimary),
        ...
        title: Column(
          ...
          children: [
            Text(
              widget.customerName.isEmpty ? 'محادثة الزبون' : widget.customerName,
              style: const TextStyle(fontFamily: 'Amiri', color: _kChatText, fontSize: 16, fontWeight: FontWeight.bold),
            ),
            Text(
              widget.locked ? 'المحادثة مغلقة' : 'متصل الآن',
              style: TextStyle(fontFamily: 'Amiri', color: widget.locked ? _kChatSub : _kChatPrimary, fontSize: 11),
            ),
          ],
        ),
      ),
      body: Column(
        children: [
          Expanded(
            child: _loading
                ? const Center(child: CircularProgressIndicator(color: _kChatPrimary))
                : _messages.isEmpty
                    ? Center(
                        child: Text(
                          widget.locked ? 'لا توجد رسائل' : 'ابدأ المحادثة مع الزبون',
                          style: const TextStyle(fontFamily: 'Amiri', color: _kChatSub, fontSize: 14),
                        ),
                      )
                    : ListView.builder(
                        controller: _scroll,
          ...
          if (widget.locked)
            Padding(
              ...
              child: Text(
                ...
                style: const TextStyle(fontFamily: 'Amiri', color: _kChatSub, fontSize: 12),
              ),
            )
          else
            Container(
                  ...
                  Expanded(
                    child: Container(
                      ...
                      child: TextField(
                        ...
                        maxLines: 4,
                        minLines: 1,
                        style: const TextStyle(fontFamily: 'Amiri', color: _kChatText, fontSize: 14),
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        color: _ctrl.text.trim().isEmpty && !_sending
                            ? _kChatPrimary.withOpacity(0.35)
                            : _kChatPrimary,
                      ),
                      child: _sending
                          ? const Padding(
  ...
  Widget _bubble(Map<String, dynamic> m) {
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isMine ? _kChatPrimary : Colors.white,
        ...
        child: Column(
          ...
          children: [
            Text(
              m['text'] ?? '',
              style: TextStyle(fontFamily: 'Amiri', color: isMine ? Colors.white : _kChatText, fontSize: 14),
            ),
            if (time.isNotEmpty)
              Padding(
                ...
                child: Text(
                  time,
                  style: TextStyle(fontFamily: 'Amiri', color: isMine ? Colors.white70 : _kChatSub, fontSize: 10),
                ),
              ),
          ],
        ),
