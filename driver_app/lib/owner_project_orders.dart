...
import 'package:dashbord/project_chat_screen.dart';
import 'package:dashbord/project_unread_store.dart';
...
class _OwnerProjectOrdersPageState extends State<OwnerProjectOrdersPage> {
  ...
  void _onProjectMessage(dynamic data) {
    ...
    if (projectId.isEmpty || fromRole != 'customer') return;
  ...
  Future<void> _loadProjects() async {
    try {
      ...
      try {
        ...
        final items = (unread['items'] as List? ?? []).cast<Map<String, dynamic>>();
    ...
    return ListView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        return ListenableBuilder(
          ...
          builder: (_, __) => _ProjectOrderCard(
            projectId: d['_id'] ?? '',
            data: d,
            storeId: widget.storeId,
            storeName: widget.storeName,
...
class _ProjectOrderCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final capacite = data['capacite'] as String? ?? '';
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        const Icon(CupertinoIcons.chat_bubble_2_fill, color: _kPrimary, size: 14),
  ...
  Future<void> _openChat(BuildContext context) async {
    ...
    final customerName = data['name'] as String? ?? 'الزبون';
    final status = data['status'] as String? ?? '';
    final locked = ['completed', 'cancelled', 'delivered'].contains(status);
...
class _ProjectOrderDetailSheetState extends State<_ProjectOrderDetailSheet> {
  ...
  Future<void> _openChat() async {
    final customerId = widget.data['userId'] as String? ?? '';
    final customerName = widget.data['name'] as String? ?? 'الزبون';
  ...
  Future<void> _acceptProject() async {
    ...
    if (method == 'self') {
      ...
      try {
        final Map<String, dynamic> delivery = await ApiClient.post('/api/project-deliveries', {
          ...
          'deliveryPrice': 0,
          'productPrice': (widget.data['productPrice'] as num? ?? 0).toDouble(),
          'totalPrice': (widget.data['productPrice'] as num? ?? 0).toDouble(),
          'storeLat': 0,
          'storeLng': 0,
          'storeAddress': '',
        ...
        // إشعار للزبون أن طلبه تم قبوله وواصلة إليه
        if (widget.data['userId'] != null) {
          await ApiClient.post('/api/notifications', {
            'toId': widget.data['userId'],
            'title': '✅ تم قبول طلبك',
            'body': 'طلبيتك تم قبولها وراهي جاية إليك',
        ...
        // حفظ التوصيلية وحالة المشروع في data باش تظهر في الشيت
        ...
        if (ctx.mounted) {
          ScaffoldMessenger.of(ctx).showSnackBar(
            SnackBar(
              content: const Text(
                '✅ تم تسجيل التوصيلية بدون سائق، أوصلي الطلبية للزبون ثم أكدي التسليم',
                style: TextStyle(fontFamily: 'Amiri'),
              ),
              backgroundColor: _kSuccess,
              behavior: SnackBarBehavior.floating,
    ...
    try {
      ...
      // إشعار للزبون أن طلبه تم قبوله
      if (widget.data['userId'] != null) {
        await ApiClient.post('/api/notifications', {
          'toId': widget.data['userId'],
          'title': '✅ تم قبول طلبك',
          'body': 'طلبيتك تم قبولها، تم إرسال التوصيلية إلى السائق',
          'type': 'project_accepted',
          'createdAt': DateTime.now().toIso8601String(),
          'isRead': false,
        });
  ...
  Future<void> _confirmSelfDelivered(BuildContext ctx) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (dCtx) => AlertDialog(
        ...
        content: const Column(
          ...
          children: [
            Text(
              "سلمي الطلبية للزبون ثم أكدي الأمر",
              textAlign: TextAlign.center,
              style: TextStyle(fontFamily: 'Amiri'),
            ),
            SizedBox(height: 8),
            Text(
              "هل سلمت الطلبية للزبون؟",
              textAlign: TextAlign.center,
    ...
    try {
      ...
      final updated = await ApiClient.put(
        '/api/project-deliveries/$deliveryId',
        {'status': 'delivered'},
      );
      await ApiClient.put('/api/projects/${widget.projectId}', {'status': 'completed'});
      if (mounted) {
        setState(() {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('✅ تم تأكيد تسليم الطلبية', style: TextStyle(fontFamily: 'Amiri')),
            backgroundColor: _kSuccess,
            behavior: SnackBarBehavior.floating,
          ),
        );
  ...
  Widget _buildDeliveryStatus() {
    ...
    switch (status) {
      ...
      case 'near_customer':
        ...
        break;
      case 'self_delivery':
        statusText = '🛵 بدون سائق — سلمي الطلبية للزبون ثم أكدي';
        statusColor = _kPrimary;
        break;
      case 'delivered':
