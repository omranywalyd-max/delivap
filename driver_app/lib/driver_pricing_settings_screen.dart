...
class _DriverPricingSettingsScreenState
  ...
  Future<void> _loadExistingConfig() async {
    ...
    try {
      ...
      // 1. نحاول نجيب التسعيرة من wilaya_configs (السيرفر) — تسعيرة الدراجة فقط
      // نجرب بالاسم العربي أولاً
      if (_cityNameAr.isNotEmpty) {
        final serverData = await ApiClient.get('/api/wilaya-configs/$_cityNameAr?vehicleType=motorcycle');
      ...
      if (cityConfig == null && _cityNameFr.isNotEmpty) {
        final serverData = await ApiClient.get('/api/wilaya-configs/$_cityNameFr?vehicleType=motorcycle');
        if (serverData is Map && serverData['basePrice'] != null) {
          cityConfig = Map<String, dynamic>.from(serverData as Map);
          fromServer = true;
        }
      ...
      if (cityConfig == null) {
        final serverData = await ApiClient.get('/api/wilaya-configs/$_cityName?vehicleType=motorcycle');
        if (serverData is Map && serverData['basePrice'] != null) {
          cityConfig = Map<String, dynamic>.from(serverData as Map);
          fromServer = true;
        }
      ...
      if (cityConfig != null) {
        // وجدنا تسعيرة للمدينة — نعبي الحقول
        // نقفّل التعديل فقط إذا السائق ما عندوش صلاحية التسعيرة
        bool driverCanEdit = false;
        try {
          final uid = DriverService.uid;
          if (uid != null) {
            final driverData = await ApiClient.get('/api/drivers/$uid');
            if (driverData is Map) {
              driverCanEdit = driverData['canSetPricing'] == true;
            }
          }
        } catch (_) {}
        setState(() {
          ...
          _cityPricingLocked = fromServer && !driverCanEdit;
        });
