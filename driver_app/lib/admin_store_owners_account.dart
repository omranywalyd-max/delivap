...
class _CategoryStoreCardState extends State<_CategoryStoreCard> {
  ...
  bool get canUploadVideo => widget.item['canUploadVideo'] == true;

  Future<void> _toggleVideoPermission() async {
    final id = widget.item['_id'];
    final newVal = !canUploadVideo;
    try {
      final res = await ApiClient.put('/api/categories/$id', {'canUploadVideo': newVal});
      if (mounted) {
        widget.item['canUploadVideo'] = res['canUploadVideo'] ?? newVal;
        setState(() {});
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(newVal ? '✅ تم تفعيل عروض الفيديو لهذا القسم' : 'تم إلغاء تفعيل عروض الفيديو', textAlign: TextAlign.center, style: const TextStyle(fontFamily: 'Amiri')),
          ...
          children: [
            ...
            Row(children: [
              ...
              Expanded(
                child: GestureDetector(
                  behavior: HitTestBehavior.opaque,
                  onTap: () => _toggleVideoPermission(),
                  child: Container(
                    padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                    decoration: BoxDecoration(
                      color: canUploadVideo ? Colors.green.withOpacity(0.12) : Colors.grey.withOpacity(0.12),
                      borderRadius: BorderRadius.circular(10),
                    ),
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(canUploadVideo ? CupertinoIcons.play_circle_fill : CupertinoIcons.play_circle, color: canUploadVideo ? Colors.green : Colors.grey, size: 14),
                      const SizedBox(width: 6),
                      Text(canUploadVideo ? 'الفيديو مفعل' : 'الفيديو', style: TextStyle(fontFamily: 'Amiri', fontWeight: FontWeight.bold, color: canUploadVideo ? Colors.green : Colors.grey, fontSize: 12)),
                    ]),
                  ),
                ),
              ),
...
class _CategoryDetailPageState extends State<_CategoryDetailPage> {
  ...
  bool get canUploadVideo => cat['canUploadVideo'] == true;
        ...
        children: [
          ...
          Container(
            ...
            child: SwitchListTile(
              ...
              secondary: Icon(canUploadVideo ? CupertinoIcons.play_circle_fill : CupertinoIcons.play_circle, color: canUploadVideo ? Colors.green : Colors.grey),
              title: Text('تفعيل عروض الفيديو', style: const TextStyle(fontFamily: 'Amiri', fontWeight: FontWeight.bold, fontSize: 14)),
              subtitle: Text(canUploadVideo ? 'القسم يقدر يضيف عروض فيديو' : 'القسم يضيف صور فقط', style: TextStyle(fontFamily: 'Amiri', fontSize: 11, color: canUploadVideo ? Colors.green : Colors.grey.shade500)),
              value: canUploadVideo,
              onChanged: (v) async {
                try {
                  final res = await ApiClient.put('/api/categories/${cat['_id']}', {'canUploadVideo': v});
                  if (mounted) {
                    cat['canUploadVideo'] = res['canUploadVideo'] ?? v;
                    setState(() {});
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text(v ? '✅ تم تفعيل عروض الفيديو' : 'تم إلغاء تفعيل عروض الفيديو', textAlign: TextAlign.center, style: const TextStyle(fontFamily: 'Amiri')),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                const Icon(CupertinoIcons.doc_text, color: _kPrimary, size: 18),
                const SizedBox(width: 8),
                Text('كشف الحساب', style: TextStyle(fontFamily: 'Amiri', fontWeight: FontWeight.bold, color: _kPrimary, fontSize: 15)),
              ]),
            ),
          ),
        ],
