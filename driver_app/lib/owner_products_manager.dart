...
const int kRequiredPromoWidth = 1100;
const int kRequiredPromoHeight = 500;
const double kRequiredPromoRatio = 2.2;
      ...
      children: [
        ...
        Text(
          ...
          style: const TextStyle(
            ...
            Center(
              child: Text(
                'الصورة لازم تكون ${kRequiredPromoWidth}×${kRequiredPromoHeight} pixel',
                style: TextStyle(
                  fontFamily: 'Amiri',
                  fontSize: 11,
...
class _OwnerOffersPageState extends State<OwnerOffersPage> {
  ...
  void _delete(BuildContext context, Map<String, dynamic> doc) async {
    showCupertinoDialog(
      context: context,
      builder: (ctx) => CupertinoAlertDialog(
        title: const Text("حذف العرض",
            style: TextStyle(fontFamily: 'Amiri')),
        content: const Text(
            "سيتم حذف هذا العرض وجميع بياناته نهائياً من التطبيق والسيرفر فوراً.",
            style: TextStyle(fontFamily: 'Amiri')),
        actions: [
          CupertinoDialogAction(
            child: const Text("إلغاء",
          ...
          CupertinoDialogAction(
            ...
            child: const Text("حذف العرض",
                style: TextStyle(fontFamily: 'Amiri')),
            onPressed: () async {
              final res = await ApiClient.delete('/api/promotions/${doc['_id']}');
              Navigator.pop(ctx);
              if (res['deleted'] == true) {
                _load();
                _showAlert(context, "✅ تم حذف العرض نهائياً");
              } else {
                _showAlert(context, "❌ فشل حذف العرض، حاول مرة أخرى");
              }
            },
          ),
        ],
...
class _OfferEditorSheetState extends State<OfferEditorSheet> {
  ...
  void initState() {
    ...
    if (widget.doc != null) {
      ...
      _existingVid = d['video'] ?? '';
      _isActive = d['isActive'] ?? true;
      _selectedCatId = d['categorieId'];
      _selectedCatName = d['categoryName'] ?? '';
    }
    _loadCategories();
  }
  ...
  Future<void> _loadCategories() async {
    try {
      final cats = await ApiClient.getList('/api/categories?storeId=${widget.storeId}');
      final list = cats.whereType<Map<String, dynamic>>().toList();
      if (!mounted) return;
      setState(() {
        _categories = list;
        if (_selectedCatId == null && list.length == 1) {
          _selectedCatId = list[0]['_id'] as String?;
          _selectedCatName = list[0]['nom'] as String? ?? '';
        }
        _canVideo = _canCategoryVideo(list, _selectedCatId);
      });
    } catch (_) {}
  }

  bool _canCategoryVideo(List<Map<String, dynamic>> cats, String? catId) {
    if (catId == null) return false;
    for (final c in cats) {
      if (c['_id'] == catId) return c['canUploadVideo'] == true;
    }
    return false;
  }
  ...
  Future<void> _pickVideo() async {
    try {
      ...
      if (length > 90 * 1024 * 1024) {
        _showAlert(context, "الفيديو كبير جداً (الحد الأقصى 90MB)");
        return;
      }
      setState(() {
        _vidFile = f;
      ...
      _showAlert(context, "تعذر اختيار الفيديو: $e");
    }
  }

  void _removeVideo() {
    final oldVid = _existingVid;
    setState(() {
      _vidFile = null;
      _existingVid = '';
    });
    if (oldVid.isNotEmpty) ApiClient.deleteImageUrl(oldVid);
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      String vidUrl = _existingVid;
      if (_vidFile != null) {
        vidUrl = await ApiClient.uploadVideo(_vidFile!);
        if (vidUrl.isEmpty) {
          if (mounted) setState(() => _loading = false);
          _showAlert(context, "فشل رفع الفيديو. تأكد من أن الحجم أقل من 90MB");
          return;
        }
      }
      // إذا حذف التاجر فيديو قديم
      final oldVid = (widget.doc?['video'] as String? ?? '');
      if (oldVid.isNotEmpty && vidUrl.isEmpty) {
        ApiClient.deleteImageUrl(oldVid);
      }
      ...
      final data = {
        ...
        'storeLat': sLat,
        'storeLng': sLng,
        'type': vidUrl.isNotEmpty ? 'video' : 'image',
        if (vidUrl.isNotEmpty) 'video': vidUrl,
        if (_selectedCatId != null) 'categorieId': _selectedCatId,
        if (_selectedCatName.isNotEmpty) 'categoryName': _selectedCatName,
        'templateName': tName,
        'updatedAt': DateTime.now().toIso8601String(),
      };
      if (widget.doc == null) {
          ...
          children: [
            ...
            GestureDetector(
              onTap: () async {
                ...
                frame.image.dispose();
                final ratio = imgW > imgH ? imgW / imgH : imgH / imgW;
                if ((ratio - kRequiredPromoRatio).abs() > 0.1) {
                  if (mounted) {
                    showDialog(
                      context: context,
                      builder: (ctx) => AlertDialog(
                        ...
                        title: const Text('صورة غير مقبولة',
                            textAlign: TextAlign.center,
                            style: TextStyle(fontFamily: 'Amiri', fontWeight: FontWeight.bold, color: Colors.red)),
                        content: Text(
                            'الصورة لازم تكون $kRequiredPromoWidth×$kRequiredPromoHeight\nالصورة الحالية: $imgW×$imgH',
                            textAlign: TextAlign.center,
                            style: const TextStyle(fontFamily: 'Amiri', fontSize: 13)),
                        actions: [
                          TextButton(
                            onPressed: () => Navigator.pop(ctx),
                            child: const Text('حسناً', style: TextStyle(fontFamily: 'Amiri', color: Color(0xFF7D29C6))),
                          ),
                        ],
                      ),
                    );
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: _vidFile != null
                            ? Text(_vidFile!.path.split('/').last.split('\\').last,
                                maxLines: 1,
                                ...
                                style: const TextStyle(
                                    ...
                                    color: kTextPrimary,
                                    fontSize: 13))
                            : _existingVid.isNotEmpty
                                ? const Text("فيديو مرفق (اضغط للتغيير)",
                                    style: TextStyle(
                                        fontFamily: 'Amiri',
                                        color: kPrimary,
                                        fontSize: 13))
                                : const Text("إضافة فيديو للعرض (اختياري)",
                                    style: TextStyle(
                                        fontFamily: 'Amiri',
                                        color: kTextSecondary,
                                        fontSize: 13)),
                      ),
                      if (_vidFile != null || _existingVid.isNotEmpty)
                        IconButton(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    items: _categories.map((c) {
                      ...
                      final name = c['nom'] as String? ?? '';
                      return DropdownMenuItem(value: id, child: Text(name,
                          style: const TextStyle(fontFamily: 'Amiri')));
                    }).toList(),
                    onChanged: (v) {
                      if (v == null) return;
                      final cat = _categories.firstWhere((c) => c['_id'] == v);
                      setState(() {
                        _selectedCatId = v;
                        _selectedCatName = cat['nom'] as String? ?? '';
                        _canVideo = cat['canUploadVideo'] == true;
                        if (!_canVideo) {
                          _vidFile = null;
                          _existingVid = '';
                        }
