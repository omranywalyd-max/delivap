...
class _MapPickerScreenState extends State<MapPickerScreen> {
  ...
  Future<void> _getUserLocation() async {
    try {
      ...
      // نجرّب آخر موقع معروف أولاً (سريع)، وإلا نجلب موقع GPS بمهلة محدودة
      ...
      if (pos == null) throw Exception('لا يوجد موقع');
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('تعذر تحديد موقعك تلقائياً، حرّك النقطة على الخريطة أو استعمل البحث'),
            backgroundColor: kWarning,
          ),
        );
        // نحدد اسم المدينة للنقطة الحالية حتى لا يضطر لتحريك النقطة
        await _reverseGeocode(_selectedLatLng);
      }
    }
  }
  ...
  Future<void> _reverseGeocode(LatLng ll) async {
    ...
    try {
      ...
      final results = await Future.wait([
        http.get(Uri.parse(urlAr), headers: {'User-Agent': 'walyyid-app'}).timeout(const Duration(seconds: 10)),
        http.get(Uri.parse(urlFr), headers: {'User-Agent': 'walyyid-app'}).timeout(const Duration(seconds: 10)),
      ]);
      ...
      } else if (mounted && _selectedAddress.isEmpty) {
        setState(() => _selectedAddress = 'موقع محدد');
      }
    } catch (e) {
      // نحتفظ بالعنوان القديم إن وُجد بدل إظهار خطأ
      if (mounted && _selectedAddress.isEmpty) {
        setState(() => _selectedAddress = 'موقع محدد');
      }
    } finally {
