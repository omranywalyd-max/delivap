  ...
  void _groupOrdersByStore() {
    ...
    for (final data in widget.activeOrders) {
      ...
      if (isProject) {
        ...
        if (data['userLat'] != null && data['userLng'] != null) {
          final uLat = (data['userLat'] as num?)?.toDouble();
          final uLng = (data['userLng'] as num?)?.toDouble();
          if (uLat != null && uLng != null) {
            custs.add({
              'name': customer,
              'latlng': LatLng(uLat, uLng),
            });
          }
        }
      }
    ...
    onTap: () { // حذفنا شرط if (!isFinished)
      ...
      storeIndex++;
        ...
        for (final cust in _customerData) {
          ...
          final LatLng custPos = cust['latlng'] as LatLng;
          markers.add(Marker(
            markerId: MarkerId('cust_${custName}_${custPos.latitude}_${custPos.longitude}'),
            position: custPos,
            icon: await _createCustomerMarker(custName, isComplete: complete),
            anchor: const Offset(0.5, 1.0),
            onTap: () => _selectCustomer(custName, custPos)));
        }
    ...
    return BitmapDescriptor.fromBytes(data!.buffer.asUint8List());
      ...
      Future<BitmapDescriptor> _createCustomerMarker(String name, {bool isComplete = false}) async {
        ...
        // الألوان: حامق إذا كامل (تم الشراء)، باهت إذا مازال المنتج
        ...
        // الجسم يبان شفاف وباهت إذا المنتجات مازال ما تم شرائها
        final List<Color> bodyColors = isComplete
            ? colors
        ...
        canvas.drawCircle(
          ...
          45,
          Paint()
            ..color = accentColor.withOpacity(isComplete ? 0.2 : 0.08)
      ...
      Future<void> _selectCustomer(String name, LatLng pos) async {
        HapticFeedback.mediumImpact();
        setState(() {
          _selectedKey          = null;
      ...
      // ── كل منتجات الزبون الواحد (من كل المحلات) ──────────────────────────────
      List<Map<String, dynamic>> _itemsForCustomer(String name) {
        final List<Map<String, dynamic>> result = [];
        for (final entry in _fullGrouped.entries) {
          final items = entry.value[name];
              ...
              children: [
                ...
                Expanded(
                  child: Stack(
                    children: [
                      _buildMap(),
                      if ((_selectedKey != null || _selectedCustomerName != null) &&
                          _cardScreenOffset != null)
                        _buildFloatingCard(),
                      _buildLegend(),
                      _buildStatsBar(),
      ...
      // ── Floating Store / Customer Card ──────────────────────────────────────────
      Widget _buildFloatingCard() {
        final offset = _cardScreenOffset!;
        final String? custName = _selectedCustomerName;

        final double zoom = (_currentZoom / 17.0).clamp(0.7, 1.4);
        final double cw   = 300.0 * zoom;
        final double ch   = 400.0 * zoom;
        ...
        final Widget card;
        if (custName != null) {
          final items = _itemsForCustomer(custName);
          card = _CustomerProductsCard(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildStaticLegendRow(color: AppColors.textMuted, label: 'زبون (مازال)', isDot: true),
                  const SizedBox(height: 4),
                  _buildStaticLegendRow(color: Colors.red, label: 'زبون (تم شراء منتجاته)', isDot: true),
                  const SizedBox(height: 4),
    ...
    // ══════════════════════════════════════════════════════════════════════════════
    //  _CustomerProductsCard — كارد زبون: كل المنتجات اللي طالبهم (الاسم + الموديل +
    //  الكاتغوري + الكمية + الحالة)
    // ══════════════════════════════════════════════════════════════════════════════
    class _CustomerProductsCard extends StatelessWidget {
      ...
      bool _isPurchased(Map<String, dynamic> item) =>
          (item['purchaseStatus'] as String? ?? '') == 'purchased';

      @override
      Widget build(BuildContext context) {
        final int purchasedCount = items.where(_isPurchased).length;
        final int pendingCount = items.length - purchasedCount;
            ...
            children: [
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: Colors.white.withOpacity(0.15),
                        shape: BoxShape.circle),
                      child: const Icon(CupertinoIcons.person_fill, color: Colors.white, size: 11)),
                  ])),
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Text(
                      pendingCount > 0 ? '$pendingCount مازال' : 'تم الشراء كلشي',
                      style: TextStyle(
                    ...
                    Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        _MiniDot(color: AppColors.success, label: '$purchasedCount تم'),
                        const SizedBox(width: 8),
                        _MiniDot(color: AppColors.warning, label: '$pendingCount مزال'),
                      ]),
                  ])),
              ...
              Flexible(
                child: items.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (_, i) =>
                            _CustomerProductTile(item: items[i]))),
              const SizedBox(height: 6),
            ]));
      }
    ...
    // ══════════════════════════════════════════════════════════════════════════════
    //  _CustomerProductTile — منتج واحد في كارد الزبون
    // ══════════════════════════════════════════════════════════════════════════════
    class _CustomerProductTile extends StatelessWidget {
      final Map<String, dynamic> item;
      const _CustomerProductTile({required this.item});
      ...
      Widget build(BuildContext context) {
        ...
        final category  = item['categoryName'] as String? ?? '';
        ...
        return Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.end,
            children: [
              // السطر الأول: الحالة + الاسم + الكمية
              Row(
                children: [
                  if (purchased)
                    _StatusBadge(
              ...
              // السطر الثاني: الكاتغوري + الموديل/الحجم + المحل
              if (category.isNotEmpty || size.isNotEmpty || storeName.isNotEmpty)
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      if (category.isNotEmpty) ...[
                        _MiniTag(label: category, color: AppColors.vibrant),
                        const SizedBox(width: 4),
                      ],
                      if (size.isNotEmpty) ...[
                        _MiniTag(label: size, color: AppColors.soft),
                        const SizedBox(width: 4),
                      ],
                      if (storeName.isNotEmpty)
                        _MiniTag(label: storeName, color: AppColors.richPurple),
                    ])),
            ]));
      }
    }
    ...
    class _MiniDot extends StatelessWidget {
      ...
      Widget build(BuildContext context) => Row(
        ...
        children: [
          Text(
            label,
            style: TextStyle(color: color, fontSize: 8, fontFamily: 'Amiri')),
          const SizedBox(width: 3),
          Container(
            width: 7,
            height: 7,
