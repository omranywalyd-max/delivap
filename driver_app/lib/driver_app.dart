...
import 'package:dashbord/driver_transport_pricing_settings_screen.dart';
...
void _showOrderImageFullscreen(BuildContext context, String url) {
  showDialog(
    context: context,
...
class DriverService {
  ...
  static Future<void> startLocationSharing() async {
    ...
    // بث الموقع الحالي فوراً حتى لو السائق واقف
    try {
...
class _DriverSignInScreenState extends State<DriverSignInScreen>
  ...
  Future<void> _signIn() async {
    ...
    try {
      ...
      // 🚗 تسعيرة النقل (سيارة / هاربين / فورغو) — سائق النقل يشوف فقط تاع الكيلومتر
      final transportVehicle = data['vehicleType'] as String? ?? '';
      final isTransportDriver = ['car', 'harbin', 'fourgon', 'minibus', 'truck']
          .contains(transportVehicle.toLowerCase());
      final hasSetTransportPricing = data['hasSetTransportPricing'] == true;

      if (isTransportDriver) {
        // سائق النقل: مطلوبة فقط تسعيرة الكيلومتر، يتخطى شاشة التوصيل بالفئات
        if (canSetPricing && !hasSetTransportPricing) {
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (_) =>
                  const DriverTransportPricingSettingsScreen(isEditMode: false),
            ),
          );
          return;
        }
      } else if (canSetPricing && !hasSetPricing) {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (_) =>
      ...
      SocketClient().init();
      SocketClient().join('driver_${DriverService.uid}');
...
class _DriverRegisterScreenState extends State<DriverRegisterScreen>
  ...
  Future<void> _detectCity() async {
    ...
    try {
      ...
      if (mounted) _showSnackBar('تأكد من تشغيل الـ GPS والإنترنت');
...
class _ProjectDeliveryCardState extends State<_ProjectDeliveryCard>
  ...
  Widget build(BuildContext context) {
    ...
    final customerAddress = d['customerAddress'] ?? '';
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        storeName.isNotEmpty ? storeName : 'صاحب المشروع',
                        style: const TextStyle(
                          fontWeight: FontWeight.bold,
                          fontSize: 13,
                          color: kTextDark,
                    ...
                    child: Column(
                      ...
                      children: [
                        if (imageUrl.isNotEmpty) ...[
                          GestureDetector(
                            onTap: () => _showOrderImageFullscreen(context, imageUrl),
                            child: Container(
                        ...
                        _infoRow(CupertinoIcons.person_fill, 'صاحبة المشروع', ownerName),
...
class _TransportOrderCard extends StatelessWidget {
  ...
  void _showDetail(BuildContext context) {
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _TransportOrderDetailSheet(
        docId: docId,
        data: data,
...
class _ServiceOrderCard extends StatelessWidget {
  ...
  void _showDetail(BuildContext context) {
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _ServiceOrderDetailSheet(
        docId: docId,
        data: data,
        vehicleType: vehicleType,
      ),
...
class _ServiceOrderDetailSheet extends StatefulWidget {
  ...
  final Map<String, dynamic> data;
  final String vehicleType;
  const _ServiceOrderDetailSheet({
    required this.docId,
    required this.data,
    this.vehicleType = '',
  });
                    ...
                    child: Column(
                      ...
                      children: [
                        if (parcelImage.isNotEmpty) ...[
                          GestureDetector(
                            onTap: () => _showOrderImageFullscreen(context, parcelImage),
                            child: Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        if (d['distanceKm'] != null)
                          _priceRow('المسافة', '${(d['distanceKm'] as num).toStringAsFixed(1)} كم', kTextGrey, bold: false, fontSize: 13),
...
class _TransportOrderDetailSheet extends StatefulWidget {
  ...
  final Map<String, dynamic> data;
  final String vehicleType;
  const _TransportOrderDetailSheet({
    required this.docId,
    required this.data,
    this.vehicleType = '',
  });
        ...
        children: [
          ...
          ClipRRect(
            ...
            child: item.image.isNotEmpty
                ? GestureDetector(
                    onTap: () => _showOrderImageFullscreen(context, item.image),
                    child: CachedNetworkImage(
                      imageUrl: item.image,
                      width: 48,
                      height: 48,
...
class DriverNotificationHelper {
  ...
  static Future<void> init(String driverId) async {
    try {
      ...
      // ✅ شلت onMessage.listen هنا لأن main.dart يتكفل بيه
      // FirebaseMessaging.onMessage.listen شغال في main.dart
                  ...
                  child: Column(
                    children: [
                      ...
                      if (driver?.canSetPricing == true &&
                          !const ['car', 'harbin', 'fourgon', 'minibus', 'truck']
                              .contains((driver?.vehicleType ?? '').toLowerCase())) ...[
                        _infoCard(
                          CupertinoIcons.money_dollar_circle,
                      ...
                      if (const ['car', 'harbin', 'fourgon', 'minibus', 'truck']
                          .contains((driver?.vehicleType ?? '').toLowerCase())) ...[
                        _infoCard(
                          ...
                          onTap: () {
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (_) =>
                                    const DriverTransportPricingSettingsScreen(
                                      isEditMode: true,
                                    ),
                              ),
                            );
