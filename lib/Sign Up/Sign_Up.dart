...
class PhoneScreenState extends State<PhoneScreen>
  ...
  Future<void> _savePhone() async {
    ...
    if (phone.length != 10) {
      setState(() => _error = 'أدخل 10 أرقام بالضبط');
      return;
    }
...
// ── اقتراحات اسم الموقع ────────────────────────────────────────────────────
class _QuickLabels extends StatefulWidget {
  final Function(String) onSelect;
  const _QuickLabels({required this.onSelect});

  @override
  State<_QuickLabels> createState() => _QuickLabelsState();
}

class _QuickLabelsState extends State<_QuickLabels> {
  int _selectedIndex = -1;

  static const _labels = ['🏠 منزل', '💼 عمل', '🏫 مدرسة', '🏥 مستشفى', '🛒 سوق'];
      ...
      children: _labels.asMap().entries.map((entry) {
        ...
        return GestureDetector(
          onTap: () {
            ...
            widget.onSelect(label.substring(2).trim());
          },
          child: AnimatedContainer(
            duration: const Duration(milliseconds: 200),
...
/// يفرض: أول خانة = 0، ثانية = 5/6/7، وأقصى طول 10
class _AlgerianPhoneFormatter extends TextInputFormatter {
  ...
  TextEditingValue formatEditUpdate(
    ...
    // ثانية خانة لازم 5 أو 6 أو 7
    if (text.length == 2 && !'567'.contains(text[1])) {
      return oldValue;
    }
    // أقصى 10 خانات
    if (text.length > 10) {
