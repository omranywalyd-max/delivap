...
// ✅ استيراد صفحة الخريطة من delivery_screen
...
// ══════════════════════════════════════════════════════════════════════════════
//  LocationProvider — يتابع الموقع المختار عالمياً
// ══════════════════════════════════════════════════════════════════════════════
...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  // ── ✅ GlobalKey للـ _PromotionsBanner للتحديث عند Pull to Refresh ──────────
  ...
  // ══════════════════════════════════════════════════════════════════════════
  //  ✅ Pull to Refresh — إعادة تحميل كل البيانات
  // ══════════════════════════════════════════════════════════════════════════
  ...
  // ══════════════════════════════════════════════════════════════════════════
  //  ✅ فتح منتقي الموقع (خريطة أو قائمة)
  // ══════════════════════════════════════════════════════════════════════════
  Future<void> _openLocationPicker({bool isAuto = false}) async {
    ...
    if (user != null) {
      // مسجل → اعرض bottom sheet فيه المواقع المحفوظة + خريطة
      _showLocationSheet();
    } else {
...
// ══════════════════════════════════════════════════════════════════════════════
//  _StoresHeaderDelegate — SliverPersistentHeader لثبات StoresWidget
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
//  ✅ [تعديل 3] _LocationChip — أيقونة حمراء + نصوص سوداء
// ══════════════════════════════════════════════════════════════════════════════
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        // ✅ الألوان الجديدة المتناسقة مع البنفسجي البارد
        ...
        return Padding(
          ...
          child: Container(
            ...
            child: Row(
              ...
              children: [
                // ── ✅ أيقونة الموقع باللون البنفسجي البارد ──────────────────
                Container(
                  width: 30,
                  height: 30,
                  decoration: BoxDecoration(
                  ...
                  child: Icon(
                    ...
                    // ✅ تغيير اللون من الأحمر إلى البنفسجي الأساسي
                    color: hasLoc
                        ? primaryPurple
                ...
                // ── ✅ النصوص بألوان هادئة (رمادي غامق بنفسجي) ──────────────
                Flexible(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
...
// ══════════════════════════════════════════════════════════════════════════════
//  ✅ _LocationBottomSheet — قائمة المواقع المحفوظة
// ══════════════════════════════════════════════════════════════════════════════
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                // ✅ زر تحديد من الخريطة (بتدرج بنفسجي)
                GestureDetector(
                  onTap: () {
...
// ══════════════════════════════════════════════════════════════════════════════
//  _UserAvatar — يستخدم UserLocal.data['gender'] مباشرة
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
//  _PromotionsBanner — ✅ [تعديل 1] أصبح Scrollable داخل SingleChildScrollView
//  ✅ [تعديل 2] أضفنا GlobalKey لدعم Pull to Refresh
// ══════════════════════════════════════════════════════════════════════════════
class _PromotionsBanner extends StatefulWidget {
  // ✅ أضفنا key للسماح بالوصول إليه من _DashboardScreenState
...
class _PromotionsBannerState extends State<_PromotionsBanner> {
  ...
  Future<void> _checkVersionAndLoad() async {
    // إذا كانت القائمة فارغة، أظهر مؤشر التحميل
    if (mounted && _cachedPromos.isEmpty) {
      setState(() => _loading = true);
    }

    // اذهب مباشرة لجلب العروض بدون فحص النسخة
  ...
  Future<void> _fetchPromotions() async {
    try {
      ...
      for (var data in promotions) {
        ...
        // حذف فوري للعروض المحذوفة من التطبيق
        if (isDeleted) {
          continue;
        }
      ...
      // 2. تحديث الكاش والقائمة المحلية
      ...
      if (mounted) {
        ...
        // بدء التمرير التلقائي إذا كان هناك أكثر من عرض
  ...
  Widget build(BuildContext context) {
    ...
    final bannerHeight = MediaQuery.of(context).size.width * 0.9 / 2.2;
    return SizedBox(
      height: bannerHeight,
...
class _BannerItem extends StatefulWidget {
  ...
  const _BannerItem({required this.index, required this.promoData});
...
class _BannerItemState extends State<_BannerItem> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool isVideo = (widget.promoData['type'] as String? ?? '') == 'video';
...
class _OfferDetailSheetState extends State<_OfferDetailSheet> {
  ...
  void _initVideo() {
    final type = widget.promo['type'] as String? ?? 'image';
    final video = widget.promo['video'] as String? ?? '';
    if (type != 'video' || video.isEmpty) return;
    final ctl = VideoPlayerController.networkUrl(Uri.parse(video));
    _videoCtl = ctl;
    ctl.setLooping(true);
    bool wasPlaying = false;
    ctl.addListener(() {
      if (!mounted) return;
      final playing = ctl.value.isPlaying;
  ...
  Future<void> _toggleVideo() async {
    ...
    if (ctl.value.isPlaying) {
      ...
      await ctl.play();
      // ضمانة ضد مشكلة Android المعروفة: إذا ما تشغّلش من المحاولة الأولى
      Future.delayed(const Duration(milliseconds: 120), () {
        if (ctl.value.isInitialized && !ctl.value.isPlaying) ctl.play();
      });
    }
  }

  Future<void> _openFullVideo() async {
    final ctl = _videoCtl;
    if (ctl == null || !ctl.value.isInitialized) return;
    ctl.pause();
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            if (_videoCtl != null && _videoReady)
                  ...
                  child: AspectRatio(
                    ...
                    child: Stack(
                      ...
                      children: [
                        ...
                        // ── زر تكبير الفيديو للشاشة الكاملة ──
                        Positioned(
                          right: 10,
                          bottom: 36,
                          child: GestureDetector(
            ...
            else if (_videoCtl != null && !_videoError)
              Container(
                height: 150,
                decoration: BoxDecoration(
                  color: const Color(0xFFDDDDDD),
              ...
              GestureDetector(
                onTap: () => _openFullImage(context, p['image'] ?? ''),
                child: Stack(
                  children: [
                    ClipRRect(
                      borderRadius: BorderRadius.circular(20),
...
// ══════════════════════════════════════════════════════════════════════════════
//  عارض الصورة كاملة الشاشة — مع زرار رجوع و تكبير
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
//  عارض الفيديو كامل الشاشة — أفقي إذا كان الفيديو أفقي، وعمودي إذا كان عمودي
// ══════════════════════════════════════════════════════════════════════════════
class _FullVideoViewer extends StatefulWidget {
  final String url;
  const _FullVideoViewer({required this.url});
...
class _FullVideoViewerState extends State<_FullVideoViewer> {
  ...
  void _toggleMute() {
    final ctl = _ctl;
    if (ctl == null) return;
    ctl.value.volume == 0 ? ctl.setVolume(1) : ctl.setVolume(0);
  }
