  ...
  static Order fromDoc(Map<String, dynamic> d, String id) {
    final items = (d['items'] as List? ?? []).map((item) {
      ...
      return OrderItem(
        ...
        selectedModelName: m['selectedModelName'] as String?,
      );
    }).toList();
    ...
    return Order(
      ...
      pricingDetails: d['pricingDetails'] as Map<String, dynamic>?,
    );
  }
}
...
class _ActiveOrdersScreenState extends State<ActiveOrdersScreen>
    ...
    try {
      ...
      if (mounted) {
        ...
        for (final o in [
          ...
          final co = o['counterOffer'] as Map<String, dynamic>?;
          if (co != null) {
  ...
  void _flashUpdateBadge() {
    ...
    if (mounted) setState(() => _showUpdateBadge = true);
...
class _OrderCardState extends State<OrderCard> {
  ...
  Future<void> _reorderToCart(
    ...
    try {
      ...
      // تحميل المشروبات أيضاً
      List<Map<String, dynamic>> apiDrinks = [];
      try {
        final raw = storeId != null && storeId.isNotEmpty
            ? await ApiClient.getList('/api/drinks?storeId=$storeId')
            : await ApiClient.getList('/api/drinks');
        apiDrinks = raw.cast<Map<String, dynamic>>();
      } catch (_) {}
      ...
      for (final item in order.items) {
        ...
        if (isDrinkItem) {
          // البحث في المشروبات
          for (final d in apiDrinks) {
            if ((d['name'] as String? ?? '') == item.name) {
              match = d;
              break;
            }
          }
...
//  OrderDetailsSheet
// ══════════════════════════════════════════════════════════════════════════════
//  تحويل الخريطة إلى نسخة JSON آمنة (يمنع رمي jsonEncode عند وجود NaN/Infinity)
// ══════════════════════════════════════════════════════════════════════════════
Map<String, dynamic> _jsonEncodeSafe(Map<String, dynamic> body) {
...
class OrderDetailsSheetState extends State<OrderDetailsSheet> {
  ...
  Future<void> _saveItems() async {
    final itemsData = _order.items
        .map(
          (i) => {
            ...
            'selectedModelName': i.selectedModelName,
          },
        )
        .toList();

    double newDeliveryFee = _order.deliveryFee;
    ...
    try {
      await ApiClient.put(
        '/api/orders/${widget.docId}',
        _jsonEncodeSafe({
          'items': itemsData,
          'subtotal': _order.subtotal,
          'deliveryFee': newDeliveryFee,
          'total': _order.subtotal + newDeliveryFee,
          'updatedAt': DateTime.now().toIso8601String(),
        }),
      );
    } catch (e, st) {
      debugPrint('[حفظ المنتجات] خطأ: $e\n$st');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('تعذر حفظ المنتجات: $e'),
  ...
  void _showEditAddress() {
    showModalBottomSheet(
      ...
      builder: (_) => _AddressPickerSheet(
        ...
        onSelected: (newAddress, lat, lng) async {
          _pendingSave = true;
          try {
            // حساب المسافة وسعر التوصيل فوراً (عملية محلية لا تحتاج شبكة)
            double newDeliveryFee = _order.deliveryFee;
            double? newDistKm;
            if (lat != null &&
                lng != null &&
                _order.storeLat != null &&
                _order.storeLng != null &&
                _order.pricingDetails != null &&
                !_order.isFreeDelivery) {
              newDistKm = DeliveryPricingService.calcDistance(
                _order.storeLat!,
                _order.storeLng!,
                lat,
                lng,
            ...
            // 1) حفظ العنوان والسعر الجديد فوراً حتى لا يتجمد الديالوغ
            _localVersion++;
            setState(() {
              _order.address = newAddress;
              _order.deliveryFee = newDeliveryFee;
              if (newDistKm != null && _order.pricingDetails != null) {
                _order.pricingDetails!['distanceKm'] = newDistKm;
              }
            });
            try {
              widget.onRefresh();
            ...
            await ApiClient.put(
              ...
              _jsonEncodeSafe({
                ...
                'deliveryFee': newDeliveryFee,
                'total': _order.subtotal + newDeliveryFee,
                if (_order.pricingDetails != null)
                  'pricingDetails': _order.pricingDetails,
                'updatedAt': DateTime.now().toIso8601String(),
              }),
            );
            // 2) تحديد اسم المدينة في الخلفية (محدودة بمهلة زمنية)
            unawaited(_recalcDeliveryFee(newAddress, lat, lng));
          } catch (e, st) {
            debugPrint('[حفظ العنوان] خطأ: $e\n$st');
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('تعذر حفظ العنوان: $e'),
                  backgroundColor: kDangerColor,
                ),
              );
            }
  ...
  List<Widget> _buildGroupedItems() {
    ...
    return groupedItems.entries.expand((entry) {
      ...
      return <Widget>[
        if (header != 'منتجات' || groupedItems.length > 1)
          Padding(
  ...
  Future<void> _recalcDeliveryFee(
    ...
    try {
      ...
      double distKm = DeliveryPricingService.calcDistance(
        _order.storeLat!,
        _order.storeLng!,
        lat,
        lng,
      ...
      setState(() {
        _order.deliveryFee = newDeliveryFee;
        _order.pricingDetails!['distanceKm'] = distKm;
        _order.pricingDetails!['cityName'] = city;
      });
      try {
        widget.onRefresh();
      } catch (_) {}
      ...
      debugPrint('[حساب التوصيل] خطأ: $e\n$st');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('تعذر تحديث أجرة التوصيل: $e'),
            backgroundColor: kDangerColor,
          ),
        );
      }
...
class _AddProductSheetState extends State<_AddProductSheet> {
      ...
      for (final doc in allProducts) {
        ...
        if (name.contains(trimmed) || tags.any((t) => t.contains(trimmed))) {
          ...
          merged[docId] = {
            ...
            'productId': d['productId'] as String? ?? docId,
            ...
            'hasPiecePrice': d['hasPiecePrice'] == true,
            'pricePerPiece': (d['pricePerPiece'] as num?)?.toDouble() ?? 0,
          };
        }
      }
      if (mounted)
  ...
  Future<void> _handleAdd(Map<String, dynamic> p) async {
    final product = Product(
      productId: p['productId'] as String? ?? p['id'] as String? ?? '',
      name: p['name'] ?? '',
      price: (p['price'] as num).toDouble(),
      imagePath: p['image'] ?? '',
      capacite: p['capacite'] as String? ?? '',
      priceAffiche: p['priceAffiche'] as String? ?? '',
      ...
      categoryId: p['categorieId'] as String? ?? '',
      uiStyle: (p['uiStyle'] as int?) ?? 1,
      models: (p['models'] as List?) ?? const [],
      toppings: (p['toppings'] as List?) ?? const [],
      sizes: (p['sizes'] as List?) ?? const [],
      variants: (p['variants'] as List?) ?? const [],
      extraImages: (p['extraImages'] as List?) ?? const [],
      hasPiecePrice: p['hasPiecePrice'] == true,
      pricePerPiece: (p['pricePerPiece'] as num?)?.toDouble() ?? 0,
    );
    if (!mounted) return;

    // ✅ الإضافة للكارد (الطلبية) — السلة ما تتلمسش هنا
    bool added = false;
    void emit(Product chosen) {
      final item = OrderItem(
        name: chosen.displayName,
        price: chosen.price,
        originalPrice: chosen.price,
        image: chosen.imagePath,
        quantity: chosen.quantity,
        uiStyle: chosen.uiStyle,
        capacite: chosen.capacite,
        categoryName: chosen.categoryName,
        templateName: chosen.templateName,
        storeName: chosen.storeName,
        storeId: chosen.storeId,
        storeLat: chosen.storeLat,
        storeLng: chosen.storeLng,
        productId: chosen.productId,
        categorieId: p['categorieId'] as String? ?? '',
        sizes: chosen.sizes,
        extraImages: chosen.extraImages,
        variants: chosen.variants,
        selectedModelName: chosen.selectedModelName,
      );
      widget.onAdd(item);
      // لا نغلق الشيت هنا مباشرة: الطفل (شيت التفاصيل/الديالوغ) يغلق نفسه
      // بنفسه بعد استدعاء onAdd/onProductSelected، وأي pop إضافي هنا يتسبب
      // في إغلاق شيت البحث + تنفيذ بقية الحلقة (المشروبات) على state مُتلف.
      // لذلك نغلق شيت البحث مرة واحدة فقط في الفريم الموالي بعدما يغلق الطفل.
      if (!added) {
        added = true;
    ...
    final uiStyle = (p['uiStyle'] as int?) ?? 1;
    if (uiStyle == 2 || uiStyle == 3) {
      ...
      if (drinks.isEmpty) {
        try {
          ...
          drinks = data
              ...
              .toList();
          DrinkCache.set(product.storeId, drinks);
        } catch (_) {}
      }
      if (!mounted) return;
      if (uiStyle == 2) {
...
class _AddressPickerSheetState extends State<_AddressPickerSheet> {
  ...
  Future<void> _confirm() async {
    ...
    final lat = _useMap
        ? _selectedLat
        : (_locations[_selectedIndex]['lat'] as num?)?.toDouble();
    final lng = _useMap
        ? _selectedLng
        : (_locations[_selectedIndex]['lng'] as num?)?.toDouble();
    // نغلق الديالوغ فوراً ولا ننتظر انتهاء الحفظ (الحفظ يخدم في الخلفية)
    if (mounted) Navigator.pop(context);
    unawaited(widget.onSelected(addr, lat, lng));
  }
