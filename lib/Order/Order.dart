...
class _OrdersTabState extends State<_OrdersTab> {
  ...
  Future<void> _loadAll() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() {
        final transportRaw = results[0].whereType<Map<String, dynamic>>();
        _transportDocs = transportRaw.where((m) {
        ...
        final serviceRaw = results[1].whereType<Map<String, dynamic>>();
        _serviceDocs = serviceRaw.where((m) {
          if (widget.statuses.contains(m['status'])) return true;
          if (widget.statuses.contains('cancelled')) {
            final rb = m['rejectedBy'];
            final hasRejected = rb is String ? rb.isNotEmpty : rb is List ? rb.isNotEmpty : false;
        ...
        final ordersRaw = results[2].whereType<Map<String, dynamic>>();
        _ordersDocs = ordersRaw.where((m) {
