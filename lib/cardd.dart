...
class DeliveryPricingService {
    ...
    try {
      ...
      for (final cfg in configs) {
        if (cfg is Map) {
          ...
          // كل مدينة قد يكون عندها تسعيرة لأكثر من مركبة → نحتفظ بها مرة واحدة
          if (nameAr.isNotEmpty && seen.add(nameAr)) {
  ...
  static Map<String, dynamic> calcFromConfig(
...
class _CartScreenState extends State<CartScreen> {
  ...
  void initState() {
    ...
    GlobalCart.provider.addListener(_onCartChanged);
  }
  ...
  void dispose() {
    _debounceTimer?.cancel();
    GlobalCart.provider.removeListener(_onCartChanged);
    super.dispose();
  }
...
class _CheckoutSheetState extends State<_CheckoutSheet> {
  ...
  Future<void> _confirm() async {
    ...
    try {
      ...
      final bool phoneHidden = userData['phoneHidden'] == true;
