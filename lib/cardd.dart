  ...
  static Future<bool> sendCounterAccepted({
    ...
    try {
      ...
      await ApiClient.post('/api/notify-driver', {
        'driverId': driverId,
        'title': '?? $title',
        'body': '$customerName ??? ????? ??????: $priceStr DZD',
  ...
  static String _serviceTitle(String serviceType) {
    switch (serviceType) {
      ...
      case 'pickup':
        return '????? ?????';
      default:
        return serviceType.isNotEmpty ? serviceType : '??? ?????';
    }
  }
...
class _CartScreenState extends State<CartScreen> {
  ...
  Widget _buildStoreGroup(String storeKey, List<Product> products) {
    ...
    final String headerName = p.categoryName.isNotEmpty
        ? (p.categoryName == '??? ???' && p.templateName.isNotEmpty
      ...
      children: [
        ...
        if (hasRealPricing && cityName.isNotEmpty)
          Padding(
            ...
            child: Text(
              '$cityName · ${distKm.toStringAsFixed(1)} ??',
              style: TextStyle(
                fontSize: 11,
                color: Colors.grey.shade500,
                fontFamily: 'Amiri',
  ...
  Future<void> _confirm() async {
    ...
    try {
      ...
      if (isStyle6) {
        ...
        if (projectResult['_id'] == null) throw Exception('?? ??? ??? ???????');
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (info != null && info['cityName'] != null)
                      Text(
                        '?????: ${info['cityName']} · ${(info['distanceKm'] as double).toStringAsFixed(1)} ??',
                        style: TextStyle(
                          fontSize: 11,
                          color: Colors.grey.shade500,
                          fontFamily: 'Amiri',
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            _buildField(_nameCtrl, '????? ??????', CupertinoIcons.person_fill),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 6),
          Text(
            _mapAddress.isNotEmpty ? _mapAddress : '?? ??? ????? ???? ???',
            style: TextStyle(
              fontFamily: 'Amiri',
              fontSize: 12,
      ...
      child: Column(
        ...
        children: [
          ...
          _row(
            ...
            '${basePrice.toInt()} DA',
            sub:
                '???? ??? ${baseDist.toInt()} ?? ?$baseCats ??? ?$baseQty ????',
          ),
          if (extraKm > 0)
            _row(
              '????? ?????? × ${extraKm.toStringAsFixed(1)} ??',
              '${extraCharge.toInt()} DA',
              sub: '${extraDistPrice.toInt()} DA ??? ?? ?????',
            ),
          if (extraCats > 0)
            _row(
              '????? ?????? × $extraCats',
              '${extraCatsCharge.toInt()} DA',
              sub:
                  '${extraCatPrice.toInt()} DA ??? ??? ????? (???? $categoriesCount ?????)',
            ),
          if (extraQty > 0)
            _row(
              '?????? ?????? × $extraQty',
              ...
              sub:
                  '${extraQtyPrice.toInt()} DA ??? ???? ????? (??????? $totalQty)',
            ),
