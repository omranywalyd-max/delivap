...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  // الرسائل ذات حقل notification تعرضها النيابة تلقائياً — نتجنب التكرار
...
Future<void> _initDeferredServices() async {
  ...
  messaging.getToken().then((fcmToken) async {
    ...
    if (user != null) {
      ApiClient.put('/api/users/${user.uid}', {'fcmToken': fcmToken})
  ...
  messaging.onTokenRefresh.listen((String newToken) async {
    ...
    if (u != null) {
      ApiClient.put('/api/users/${u.uid}', {'fcmToken': newToken})
          .catchError((_) => <String, dynamic>{});
      try {
...
Future<void> _showLocalNotification(RemoteMessage message) async {
  ...
  if (title == null && body == null) return;
  final soundName = message.data['sound'] as String?;
  final bool hasCustomSound = soundName != null && soundName != 'delivered';
  final channelId = hasCustomSound ? 'user_channel_$soundName' : (soundName == 'delivered' ? 'user_channel_delivered_v2' : 'user_channel');
  final channelName = hasCustomSound ? 'إشعارات $soundName' : 'إشعارات المستخدم';
  ...
  final androidDetails = AndroidNotificationDetails(
    ...
    showWhen: true,
    icon: '@mipmap/ic_launcher',
    sound: hasCustomSound ? RawResourceAndroidNotificationSound(soundName == 'ring' ? 'okhrej' : soundName) : null,
  );
