...
class Product {
  ...
  String get displayName =>
      selectedModelName != null ? '$name - $selectedModelName' : name;
...
class DrinkItem {
  ...
  factory DrinkItem.fromMap(Map<String, dynamic> d) {
    ...
    final flavors = rawFlavors.map((f) {
      ...
      final List rawSizes = fm['sizes'] ?? [];
      final sizes = rawSizes.map((s) {
        final Map<String, dynamic> sm = Map<String, dynamic>.from(s as Map);
        return DrinkSize(label: sm['label'] ?? '', price: (sm['price'] as num? ?? 0).toDouble());
      }).toList();
      return DrinkFlavor(label: fm['label'] ?? '', image: fm['image'] ?? '', sizes: sizes);
    }).toList();
...
class _CacheEntry {
  ...
  _CacheEntry(this.data, {this.lastDoc}) : time = DateTime.now();
...
class _ProdCache {
  ...
  static Map<String, dynamic>? getLastDoc(String s, String c) {
    final entry = _store[_key(s, c)];
    return entry?.lastDoc;
  }
  static void set(String s, String c, List<Product> p, {Map<String, dynamic>? lastDoc}) {
    if (_store.length >= _maxEntries) {
      final key = _store.keys.first;
      _store.remove(key);
    }
    _store.removeWhere((_, e) => !e.isValid);
    _store[_key(s, c)] = _CacheEntry(p, lastDoc: lastDoc);
  }
...
class _ProductsListScreenState extends State<ProductsListScreen>
  ...
  Future<void> _fetchFirstPage() async {
    ...
    if (_ProdCache.has(widget.storeId, widget.categoryId)) {
      ...
      if (mounted) {
        final cachedLastDoc = _ProdCache.getLastDoc(widget.storeId, widget.categoryId);
        final cachedHasMore = cachedLastDoc != null && cached.length >= _ProdCache.pageSize;
        setState(() {
  ...
  Future<void> _silentRevalidate() async {
    try {
  ...
  Future<void> _fetchFromServer({bool isInitial = true}) async {
    try {
      ...
      final Map<String, dynamic>? lastDoc = data.isNotEmpty ? Map<String, dynamic>.from(data.last as Map) : null;
      if (lastDoc != null) _lastDoc = lastDoc;
      _hasMore = data.length == _ProdCache.pageSize;
  ...
  Future<void> _fetchMore() async {
    ...
    try {
      ...
      final Map<String, dynamic>? newLastDoc = data.isNotEmpty ? Map<String, dynamic>.from(data.last as Map) : null;
      if (newLastDoc != null) _lastDoc = newLastDoc;
      _hasMore = data.length == _ProdCache.pageSize;
      final newProds = _toProducts(data.map((e) => Map<String, dynamic>.from(e as Map)).toList());
      final existingIds = _allProducts.map((p) => p.productId).toSet();
      final unique = newProds.where((p) => !existingIds.contains(p.productId)).toList();
      if (unique.isEmpty) _hasMore = false;
      final all = [..._allProducts, ...unique];
      _ProdCache.set(widget.storeId, widget.categoryId, all, lastDoc: _lastDoc);
  ...
  Future<void> _fetchDrinks() async {
    ...
    try {
      ...
      final drinks = drinksData.map((d) => DrinkItem.fromMap(Map<String, dynamic>.from(d as Map))).toList();
  ...
  void _onScroll() {
    if (_scrollThrottle || _loadingMore || !_hasMore) return;
    if (_scrollCtrl.position.pixels >= _scrollCtrl.position.maxScrollExtent - 300) {
      _scrollThrottle = true;
      _fetchMore().whenComplete(() {
  ...
  Widget _buildNormalGrid() {
    ...
    return SliverPadding(
      ...
      sliver: SliverGrid(
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: _twoColumnView ? 2 : 3, mainAxisSpacing: 20, crossAxisSpacing: _twoColumnView ? 24 : 12, childAspectRatio: _twoColumnView ? 0.78 : 0.60),
...
class _PizzaDetailSheetState extends State<PizzaDetailSheet>
  ...
  // ✅ لو الطلبية: نرجع المنتجات للكارد، لا نلمس السلة
  if (widget.onProductSelected != null) {
    widget.onProductSelected!(pizzaProduct);
    for (final dP in drinkProducts) widget.onProductSelected!(dP);
    Navigator.pop(context);
    return;
  }
  ...
  for (final dP in drinkProducts) GlobalCart.provider.toggle(dP);
...
class Style4DetailSheet extends StatefulWidget {
  ...
  final void Function(Product)? onProductSelected;
  const Style4DetailSheet({required this.product, this.onProductAddedToTemplate, this.onProductSelected});
...
class Style5DetailSheet extends StatefulWidget {
  ...
  final void Function(Product)? onProductAddedToTemplate;
  final void Function(Product)? onProductSelected;
  const Style5DetailSheet({required this.product, this.onProductAddedToTemplate, this.onProductSelected});
...
class Style6DetailSheet extends StatefulWidget {
  ...
  final void Function(Product)? onProductAddedToTemplate;
  final void Function(Product)? onProductSelected;
  const Style6DetailSheet({required this.product, this.onProductAddedToTemplate, this.onProductSelected});
...
class Style7DetailSheet extends StatefulWidget {
  ...
  final void Function(Product)? onProductAddedToTemplate;
  final void Function(Product)? onProductSelected;
  const Style7DetailSheet({required this.product, this.onProductAddedToTemplate, this.onProductSelected});
...
class Style8DetailSheet extends StatefulWidget {
  ...
  final void Function(Product)? onProductAddedToTemplate;
  final void Function(Product)? onProductSelected;
  const Style8DetailSheet({required this.product, this.onProductAddedToTemplate, this.onProductSelected});
...
// ══════════════════════════════════════════════════════════════════════════════
class _StaggeredProductCard extends StatelessWidget {
  final Product product;
  final int index;
  ...
  @override
  Widget build(BuildContext context) => ProductCardWidget(
        product: product,
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Flexible(
                        child: Text(widget.product.priceAffiche,
                          overflow: TextOverflow.ellipsis,
...
class _StaggeredNetworkImage extends StatelessWidget {
