...
// ══════════════════════════════════════════════════════════════════════════════
//  الثوابت المشتركة (نفس ألوان السلة)
// ══════════════════════════════════════════════════════════════════════════════
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            "احفظ طلبيتك من السلة للاستخدام لاحقاً",
            style: TextStyle(
              fontFamily: 'Amiri',
              color: Colors.grey.shade400,
              fontSize: 12)),
...
Widget _buildBottomSummary() {
    double subtotal = _totalPrice; // سعر المنتجات فقط
    double total = subtotal + deliveryFee; // الإجمالي مع التوصيل
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // صف سعر المنتجات
          _buildPriceRow("سعر المنتجات", formatPrice(subtotal), Colors.black87),
          const SizedBox(height: 8),

          // صف سعر التوصيل
          _buildPriceRow("سعر التوصيل", formatPrice(deliveryFee), Colors.black54),
  ...
  // دالة مساعدة لرسم صفوف الأسعار (نفس التي استعملتها في السلة)
  ...
  void _reorderFromTemplate() async {
    ...
    try {
      ...
      for (final item in items) {
        ...
        bool isActive = true;
        bool isDrink = pid.startsWith('drink_');

        // 1. البحث عن المنتج أو العرض
        if (pid.isNotEmpty) {
          if (isDrink) {
            // المشروبات: استخراج الـ drinkId و البحث في /api/drinks/
            final parts = pid.split('_');
            if (parts.length >= 2) {
              final drinkId = parts[1];
              try {
                match = await ApiClient.get('/api/drinks/$drinkId') as Map<String, dynamic>?;
              } catch (_) {}
            }
          } else {
            try {
            ...
            // إذا ما لقاه كـ منتج، جرب كـ عرض
            if (match == null) {
              try {
                match =
        ...
        // للمشروبات: إذا ما لقيناه في قاعدة البيانات، نبنيه من البيانات المحفوظة
        if (match == null && isDrink) {
          toAdd.add(Product(
            productId: pid,
            name: item['name'] as String? ?? '',
            price: ((item['prix'] ?? item['price'] ?? 0) as num).toDouble(),
            imagePath: item['image'] as String? ?? '',
            capacite: item['capacite'] as String? ?? '',
        ...
        // 2. التحقق من الحالة (محذوف/غير نشط)
        if (match['isDeleted'] == true || match['isActive'] == false) {
          notFound.add(item['name'] as String? ?? '');
          continue;
        }

        // 3. بناء المنتج بكل الحقول
        toAdd.add(Product(
          ...
          templateName: item['templateName'] as String? ?? '',
          categoryName: item['categoryName'] as String? ?? '',
          categoryId: (match['categorieId'] as String?) ?? (item['categoryId'] as String?) ?? '',
      ...
      if (context.mounted) {
        ...
        String msg = '✅ تمت إضافة ${toAdd.length} منتجات إلى السلة';
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('❌ تعذرت إعادة الطلب: $e',
              style: const TextStyle(fontFamily: 'Amiri', fontSize: 13)),
...
// ══════════════════════════════════════════════════════════════════════════════
//  ودجت البحث والإضافة — بحث متقدم مع debounce
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
//  ودجت تأكيد الطلبية المحفوظة (نفس تصميم _CheckoutSheet في cardd.dart)
// ══════════════════════════════════════════════════════════════════════════════
...
class _CheckoutSheetFromTemplateState extends State<CheckoutSheetFromTemplate> {
  ...
  Future<void> _confirmOrder() async {
    ...
    try {
      ...
      final String userName = apiName.isNotEmpty ? apiName : (FirebaseAuth.instance.currentUser?.displayName ?? 'زبون');
      ...
      final itemsData = widget.items
          .map(
            (item) => {
              ...
              'templateName': item['templateName'] ?? 'نشاط عام',
        ...
        child: _isLoading
            ...
            : Row(
                ...
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(
                    _canConfirm ? "تأكيد الطلبية الآن" : "اختر موقع التوصيل",
                    style: const TextStyle(
                      fontSize: 16,
                      color: Colors.white,
                      fontWeight: FontWeight.bold,
...
// ══════════════════════════════════════════════════════════════════════════════
//  ديالوغ النجاح (مطابق لـ cardd.dart)
// ══════════════════════════════════════════════════════════════════════════════
