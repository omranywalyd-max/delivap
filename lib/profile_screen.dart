...
import 'driver_name_cache.dart';
import 'Sign in/auth_service.dart';

// ── Colors (نفس الداشبورد) ────────────────────────────────────────────────────
...
const _kSuccess = Color(0xFF27AE60); // 👈 إضافة اللون الأخضر
...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  void initState() {
    ...
    DriverNameCache.load().then((_) => _loadUser()).then((_) => _lastRefresh = DateTime.now());
    _loadSavedLocations();
  }
  ...
  Future<void> _loadDriverNames() async {
    ...
    for (final id in driverIds) {
      ...
      final cached = DriverNameCache.getName(id);
      if (cached != null && cached.isNotEmpty && cached != 'سائق') {
        _driverNames[id] = cached;
        continue;
      }
      try {
        ...
        if (d.isNotEmpty) {
          ...
          _driverNames[id] = name.isNotEmpty ? name : 'سائق';
        } else {
          _driverNames[id] = 'سائق';
        }
      } catch (_) {
        _driverNames[id] = 'سائق';
      }
      if (_driverNames[id] != 'سائق') {
        DriverNameCache.save(id, _driverNames[id]!);
      }
    }
    if (mounted) setState(() {});
  }
  ...
  Future<void> _loadSavedLocations() async {
    try {
      ...
      if (uid != null) {
        ...
        if (mounted) setState(() => _savedLocations = list.whereType<Map<String, dynamic>>().toList());
      }
    } catch (_) {}
  }
  ...
  // ══════════════════════════════════════════════════════════════════════
  //  كارد برنامج المكافآت (النسخة المحسّنة) ✅
  // ══════════════════════════════════════════════════════════════════════
  Widget _buildDriversLoyaltyCard() {
    ...
    // أفضل تقدم بين كل السواقين (باش نعرضو preview صغير)
    int bestProgress = 0;
    for (final id in driverIds) {
              ...
              children: [
                ...
                Column(
                  ...
                  children: [
                    ...
                    Text(
                      readyCount > 0
                          ? '🎁 عندك $readyCount توصيلة مجانية جاهزة'
                          : driverIds.isEmpty
                              ? 'اطلب وجمع نقاط مع سواقينك المفضلين'
                              : '$bestProgress من 5 طلبيات مع أقرب سائق',
  ...
  Widget _buildLastRefresh() {
    ...
    if (diff.inSeconds < 60) {
      text = 'آخر تحديث: منذ ${diff.inSeconds} ثانية';
    } else if (diff.inMinutes < 60) {
      text = 'آخر تحديث: منذ ${diff.inMinutes} دقيقة';
    } else {
      text = 'آخر تحديث: منذ ${diff.inHours} ساعة';
    }
    return Center(
      child: Padding(
          ...
          Row(
            children: [
              ...
              Column(
                ...
                children: [
                  Text(
                    fullName.isNotEmpty ? fullName : 'المستخدم',
  ...
  Widget _buildInfoSection({
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.end,
      children: [
        _sectionTitle('معلومات الحساب', CupertinoIcons.person_fill),
        const SizedBox(height: 14),
        _infoRow(
          CupertinoIcons.phone_fill,
          'رقم الهاتف',
          phone.isNotEmpty ? phone : 'غير محدد'),
        const _NeumDivider(),
        _infoRow(CupertinoIcons.mail_solid, 'البريد الإلكتروني', email),
        const _NeumDivider(),
        _infoRow(
        ...
        children: [
          Row(
            ...
            children: [
              ...
              _sectionTitle('مواقعي المحفوظة', CupertinoIcons.location_fill),
            ]),
          const SizedBox(height: 14),
 ...
 // ابحث عن هاد الدالة وزيد مرر البيانات (data)
void _openLocationDialog(String uid, {String? docId,    Map<String, dynamic>? data,}) {
  showDialog(
    ...
    builder: (_) => _LocationDialog(
      userId: uid,
      docId: docId,
      initialData: data , // مرر الداتا كاملة هنا
    ),
  );
}
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('تم حذف الحساب بنجاح', style: TextStyle(fontFamily: 'Amiri')),
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('خطأ: ${AuthService.errorMessage(e.code)}', style: TextStyle(fontFamily: 'Amiri')),
  ...
  Future<void> _reauthenticateAndDelete(BuildContext context) async {
    ...
    if (providers.contains('google.com')) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('يرجى إعادة تسجيل الدخول لتأكيد هويتك', style: TextStyle(fontFamily: 'Amiri')),
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('فشل التحقق، حاول مرة أخرى', style: TextStyle(fontFamily: 'Amiri')),
              backgroundColor: Colors.red));
        }
      }
    } else {
      ...
      final ok = await showCupertinoDialog<bool>(
        ...
        builder: (ctx) => CupertinoAlertDialog(
          title: const Text('تأكيد الهوية', style: TextStyle(fontFamily: 'Amiri')),
          content: Column(
            children: [
              const Text('أدخل كلمة السر لتأكيد حذف الحساب', style: TextStyle(fontFamily: 'Amiri', fontSize: 13)),
              const SizedBox(height: 12),
              CupertinoTextField(
          ...
          actions: [
            ...
            CupertinoDialogAction(
              ...
              child: const Text('تأكيد', style: TextStyle(fontFamily: 'Amiri'))),
          ],
        ));
      if (ok != true) return;
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('كلمة السر غير صحيحة', style: TextStyle(fontFamily: 'Amiri')),
              backgroundColor: Colors.red));
        }
      } finally {
      ...
      builder: (_) => CupertinoAlertDialog(
        ...
        actions: [
          ...
          CupertinoDialogAction(
            ...
            child: const Text('خروج', style: TextStyle(fontFamily: 'Amiri'))),
        ]));
  }
              ...
              content: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    const Text(
                      "يجب عليك تسجيل الدخول أولاً للوصول إلى مميزات حسابك الشخصي",
  ...
  Future<void> _save() async {
    ...
    if (_passCtrl.text.isNotEmpty) {
      if (_passCtrl.text.length < 6) {
        _snack('كلمة السر أقل من 6 أحرف', error: true);
        return;
      }
      if (_passCtrl.text != _confirmPassCtrl.text) {
        _snack('كلمتا السر غير متطابقتين', error: true);
        return;
      }
    }
...
// ══════════════════════════════════════════════════════════════════════════════
//  _LocationDialog  ✅ تصميم محسّن بالكامل
// ══════════════════════════════════════════════════════════════════════════════
class _LocationDialog extends StatefulWidget {
  ...
  final String? initialAddress;
    final Map<String, dynamic>? initialData; // نغير هادي
...
String _cityFr = '';
  ...
  void initState() {
    super.initState();
    // ✅ هنا نعمرو كلش من البيانات القديمة باش ما يروحوش كي نديرو Update
    if (widget.initialData != null) {
      ...
      _housingType = widget.initialData!['housingType'] ?? 'منزل';
      _selectedFloor = widget.initialData!['floor'] ?? 'الطابق الأرضي';
      if (_selectedFloor == 'أرضي') _selectedFloor = 'الطابق الأرضي';
      for (final f in ['الأول', 'الثاني', 'الثالث', 'الرابع', 'الخامس', 'السادس', 'السابع', 'الثامن', 'التاسع', 'العاشر']) {
        if (_selectedFloor == f) _selectedFloor = 'الطابق $f';
      }
  ...
  Future<void> _save() async {
    if (_labelCtrl.text.trim().isEmpty || _mapAddress.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text(
            "يرجى تسمية الموقع وتحديده من الخريطة",
            style: TextStyle(fontFamily: 'Amiri'),
            textAlign: TextAlign.center),
    ...
    try {
      ...
      if (_imageFile != null) {
        ...
        if (imageUrl.isEmpty) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text("فشل رفع الصورة، تأكد من الاتصال",
                style: TextStyle(fontFamily: 'Amiri'),
                textAlign: TextAlign.center),
              backgroundColor: Colors.red,
              behavior: SnackBarBehavior.floating,
      ...
      final data = {
        ...
        'floor': _housingType == 'شقة' ? _selectedFloor : 'أرضي',
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              // ── اسم المكان ───────────────────────────────────────────
              _sectionLabel('اسم المكان', CupertinoIcons.tag_fill),
              const SizedBox(height: 8),
              _styledField(
                _labelCtrl,
                'مثال: منزلي، العمل...',
              ...
              // ── الموقع الجغرافي ──────────────────────────────────────
              _sectionLabel('الموقع الجغرافي', CupertinoIcons.map_fill),
              const SizedBox(height: 8),
              GestureDetector(
                // ابحث عن هذا الجزء داخل GestureDetector الخاص بالخريطة في ملف profile_screen.dart
                onTap: () async {
  ...
  if (res != null) {
    setState(() {
      ...
      // ✅ تأكد أن MapPickerScreen يرجعcityNameAr و cityNameFr
      ...
      _cityFr = res['cityNameFr'] ?? '';
              ...
              // ── نوع السكن ────────────────────────────────────────────
              _sectionLabel('نوع السكن', CupertinoIcons.house_fill),
              const SizedBox(height: 10),
              Row(
                children: [
                  Expanded(
                    child: _housingCard('شقة', CupertinoIcons.building_2_fill)),
                  const SizedBox(width: 12),
                  Expanded(
                    child: _housingCard('منزل', CupertinoIcons.house_fill)),
                ]),

              // ── الطابق (يظهر فقط للشقة) ──────────────────────────────
              AnimatedSize(
                duration: const Duration(milliseconds: 300),
                curve: Curves.easeOutCubic,
                child: _housingType == 'شقة'
                    ? Column(
                        crossAxisAlignment: CrossAxisAlignment.end,
                        children: [
                          const SizedBox(height: 16),
              ...
              // ── لون الباب + رقم الباب ─────────────────────────────────
              _sectionLabel('معلومات الباب', CupertinoIcons.lock_fill),
              const SizedBox(height: 8),
              Row(
                children: [
                  Expanded(
              ...
              children: [
                ...
                Text(
                  ...
                  style: const TextStyle(
                    ...
                    fontFamily: 'Amiri')),
                Text(
                  "باب ${data['doorColor']} رقم ${data['doorNumber']}",
                  style: const TextStyle(
                    fontSize: 10,
                    color: Colors.grey,
                    fontFamily: 'Amiri')),
...
// ══════════════════════════════════════════════════════════════════════════════
//  DriversLoyaltyPage — صفحة تقدم الولاء لكل سائق (النسخة المحسّنة) ✅
// ══════════════════════════════════════════════════════════════════════════════
class DriversLoyaltyPage extends StatelessWidget {
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  readyCount > 0
                      ? 'عندك $readyCount توصيلة مجانية 🎉'
                      : 'كل 5 طلبيات = توصيلة مجانية',
                  textAlign: TextAlign.right,
                  style: const TextStyle(
                    color: Colors.white,
                    fontSize: 14,
                ...
                const SizedBox(height: 4),
                Text(
                  'اطلب من نفس السائق باش تجمع نقاط أسرع',
                  textAlign: TextAlign.right,
                  style: TextStyle(
                    color: Colors.white.withOpacity(0.85),
                    fontSize: 11,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              'كل 5 طلبيات مكتملة مع نفس السائق تمنحك توصيلة مجانية',
              textAlign: TextAlign.center,
              style: TextStyle(
                fontSize: 13,
  ...
  Widget _driverCard(
    ...
    final name = driverNames[id] ?? 'سائق';
