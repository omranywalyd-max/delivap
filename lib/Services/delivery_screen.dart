...
import 'package:flutter_application_1/driver_name_cache.dart';
  ...
  class _ServiceOrderScreenState extends State<ServiceOrderScreen>
    ...
    // التسعيرة التلقائية (المسافة + تسعيرة مدينة الموقع الأول)
    bool _calculatingPrice = false;
    int _calcSeq = 0;           // لتجاهل نتائج الطلبات القديمة عند تغيير الموقع بسرعة
    double? _minPrice;          // الحد الأدنى المحسوب — الزبون يزيد فقط ولا ينقص
    Map<String, dynamic>? _pricingInfo; // تفاصيل التسعيرة (للعرض + للحفظ في الطلب)
    ...
    (double?, double?) get _fromCoords {
      if (_fromUseMap) return (_fromLat, _fromLng);
      if (_fromLocationIndex >= 0 && _fromLocationIndex < _savedLocations.length) {
        final lat = (_savedLocations[_fromLocationIndex]['lat'] as num?)?.toDouble();
        final lng = (_savedLocations[_fromLocationIndex]['lng'] as num?)?.toDouble();
        return (lat, lng);
      }
      return (null, null);
    }

    (double?, double?) get _toCoords {
      if (_toUseMap) return (_toLat, _toLng);
      if (_toLocationIndex >= 0 && _toLocationIndex < _savedLocations.length) {
        final lat = (_savedLocations[_toLocationIndex]['lat'] as num?)?.toDouble();
        final lng = (_savedLocations[_toLocationIndex]['lng'] as num?)?.toDouble();
        return (lat, lng);
      }
      return (null, null);
    }

    /// يحسب المسافة بين الموقعين والتسعيرة حسب مدينة الموقع الأول (بالكيلومتر)
    Future<void> _autoCalcPrice() async {
      final from = _fromCoords;
      final to = _toCoords;
      final fLat = from.$1;
      final fLng = from.$2;
      final tLat = to.$1;
      final tLng = to.$2;
      final hasCoords = fLat != null && fLat != 0 && fLng != null && fLng != 0 &&
          tLat != null && tLat != 0 && tLng != null && tLng != 0;
      final mySeq = ++_calcSeq;
      if (!hasCoords) {
        if (mounted) setState(() {
          _minPrice = null;
          _pricingInfo = null;
      ...
      try {
        final res = await ApiClient.post('/api/service-orders/quote', {
          'fromLat': fLat,
          'fromLng': fLng,
          'toLat': tLat,
          'toLng': tLng,
    ...
    Future<void> _confirmOrder() async {
      ...
      try {
        ...
        final phoneHidden = userData['phoneHidden'] == true;
        ...
        final orderData = {
          ...
          'userCityName': _fromCityAr.isNotEmpty
              ? _fromCityAr
              : (userData['cityNameAr'] as String? ?? ''),
          'userCityNameFr': _fromCityFr.isNotEmpty
              ? _fromCityFr
              : (userData['cityNameFr'] as String? ?? ''),
          'distanceKm': (_pricingInfo?['distanceKm'] as num?)?.toDouble(),
    ...
    Widget _buildPriceField() {
      final pricing = _pricingInfo;
      final minPrice = _minPrice;
      final distKm = (pricing?['distanceKm'] as num?)?.toDouble();
      final cityName = (pricing?['cityName'] as String?) ?? '';
      final basePrice = (pricing?['basePrice'] as num?)?.toDouble() ?? 0;
      final extraDistPrice = (pricing?['extraDistPrice'] as num?)?.toDouble() ?? 0;
              ...
              child: Column(
                ...
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text('الحد الأدنى: ${minPrice.toInt()} DA',
                          style: const TextStyle(fontSize: 12, fontFamily: 'Amiri', fontWeight: FontWeight.bold, color: kPrimaryColor)),
                      if (_calculatingPrice)
                        const Text('جاري الحساب...',
                            style: TextStyle(fontSize: 11, fontFamily: 'Amiri', color: Colors.black45))
                      else if (cityName.isNotEmpty)
                        Text('مدينة: $cityName',
                            style: const TextStyle(fontSize: 11, fontFamily: 'Amiri', color: Colors.black45)),
                    ],
                  ),
                  if (distKm != null) ...[
                    const SizedBox(height: 4),
                    Text(
                      'المسافة: ${distKm.toStringAsFixed(1)} كم — الأساس ${basePrice.toInt()} DA + ${extraDistPrice.toInt()} DA/كم',
                      style: const TextStyle(fontSize: 11, fontFamily: 'Amiri', color: Colors.black54),
                    ),
                  ],
                ],
              ),
        ...
        children: [
          ...
          GestureDetector(
            onTap: () async {
              ...
              if (res != null && mounted) {
                final resCityAr = res['cityNameAr'] as String? ?? '';
                final resCityFr = res['cityNameFr'] as String? ?? '';
                if (isFrom) {
                  _fromCityAr = resCityAr;
                  _fromCityFr = resCityFr;
                }
  ...
  class _TransportOrderScreenState extends State<TransportOrderScreen>
    ...
    // التسعيرة التلقائية (المسافة + تسعيرة المدينة/السائق المسؤول)
    bool _calculatingPrice = false;
    int _calcSeq = 0;           // لتجاهل نتائج الطلبات القديمة عند تغيير الموقع بسرعة
    double? _minPrice;          // الحد الأدنى المحسوب — الزبون يزيد فقط ولا ينقص
    Map<String, dynamic>? _pricingInfo; // تفاصيل التسعيرة (للعرض + للحفظ في الطلب)
    ...
    (double?, double?) get _fromCoords {
      ...
      if (_fromIndex >= 0 && _fromIndex < _savedLocations.length) {
        final lat = (_savedLocations[_fromIndex]['lat'] as num?)?.toDouble();
        final lng = (_savedLocations[_fromIndex]['lng'] as num?)?.toDouble();
        return (lat, lng);
      }
      return (null, null);
    }
    ...
    (double?, double?) get _toCoords {
      if (_toMap) return (_toLat, _toLng);
      if (_toIndex >= 0 && _toIndex < _savedLocations.length) {
        final lat = (_savedLocations[_toIndex]['lat'] as num?)?.toDouble();
        final lng = (_savedLocations[_toIndex]['lng'] as num?)?.toDouble();
        return (lat, lng);
      }
      return (null, null);
    }

    /// يُستدعى بعد تغيير أحد الموقعين لإعادة حساب التسعيرة التلقائية
    /// يحسب المسافة بين الموقعين وتسعيرة النقل حسب السائق المسؤول عن المدينة
    Future<void> _autoCalcPrice() async {
      final from = _fromCoords;
      final to = _toCoords;
      final fLat = from.$1;
      ...
      setState(() => _calculatingPrice = true);
      try {
        final res = await ApiClient.post('/api/transport-orders/quote', {
          'fromLat': fLat,
          'fromLng': fLng,
          'toLat': tLat,
          'toLng': tLng,
    ...
    Future<void> _confirmOrder() async {
      ...
      try {
        ...
        final orderData = {
          ...
          'phoneHidden': userData['phoneHidden'] == true,
    ...
    Widget _buildPriceField() {
      ...
      final basePrice = (pricing?['basePrice'] as num?)?.toDouble() ?? 0;
      final extraDistPrice = (pricing?['extraDistPrice'] as num?)?.toDouble() ?? 0;
      final isDefault = (pricing?['isDefault'] as bool?) ?? false;
              ...
              child: Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text('الحد الأدنى: ${minPrice.toInt()} DA',
                          style: TextStyle(fontSize: 12, fontFamily: 'Amiri', fontWeight: FontWeight.bold, color: _accentColor)),
                      if (_calculatingPrice)
                        const Text('جاري الحساب...',
                            style: TextStyle(fontSize: 11, fontFamily: 'Amiri', color: Colors.black45))
                      else if (cityName.isNotEmpty)
                  ...
                  if (isDefault) ...[
                    const SizedBox(height: 4),
                    const Text(
                      'لم يضبط السائق تسعيرته بعد — يُحسب بسعر افتراضي (200 DA للكم الأول + 50 DA لكل كم إضافي)',
                      style: TextStyle(fontSize: 10, fontFamily: 'Amiri', color: Colors.orange),
                    ),
                  ],
                ],
              ),
  ...
  class _ServiceDriverSelectionScreenState
    ...
    Future<void> _loadCities() async {
      try {
        String uCityAr = (widget.orderData['userCityName'] as String? ?? '').trim();
        String uCityFr = (widget.orderData['userCityNameFr'] as String? ?? '').trim();
        if (uCityAr.isEmpty) {
          uCityAr = (UserLocal.data?['cityNameAr'] as String? ?? '').trim();
          uCityFr = (UserLocal.data?['cityNameFr'] as String? ?? '').trim();
        }

        // إذا المدينة غير معروفة، نستخرجها من إحداثيات الانطلاق
        if (uCityAr.isEmpty) {
          final lat = (widget.orderData['fromLat'] as num?)?.toDouble();
          final lng = (widget.orderData['fromLng'] as num?)?.toDouble();
          if (lat != null && lng != null) {
            try {
              ...
              final resp = await http.get(
                Uri.parse(url),
                headers: {'User-Agent': 'deliveryyy-app/1.0'})
                  .timeout(const Duration(seconds: 6));
              if (resp.statusCode == 200) {
                final json = jsonDecode(resp.body) as Map<String, dynamic>;
                final addr = json['address'] as Map<String, dynamic>? ?? {};
                uCityAr = (addr['city'] as String? ??
                    addr['town'] as String? ??
        ...
        if (mounted) {
          ...
          // مطابقة تامة أولاً، ثم مطابقة جزئية
          ...
          if (autoSelect == null) {
            for (final c in allCities) {
              final cn = normalize(c);
              if (uAr.isNotEmpty && (cn.contains(uAr) || uAr.contains(cn))) {
                autoSelect = c;
                break;
              }
              if (uFr.isNotEmpty && (cn.contains(uFr) || uFr.contains(cn))) {
                autoSelect = c;
                break;
              }
            }
    ...
    Future<void> _loadDrivers() async {
      try {
        ...
        if (mounted) {
          ...
          for (final d in _driversList) {
            final id = d['uid'] as String? ?? '';
            final fn = d['firstName'] as String? ?? '';
            final ln = d['lastName'] as String? ?? '';
            final name = '$fn $ln'.trim();
            if (id.isNotEmpty && name.isNotEmpty) namesToCache[id] = name;
          }
    ...
    Widget _buildDriversContent() {
      ...
      String uCityAr = normalize(_userCityAr.isNotEmpty
          ? _userCityAr
          : (UserLocal.data?['cityNameAr'] as String? ?? ''));
      String uCityFr = normalize(_userCityFr.isNotEmpty
          ? _userCityFr
          : (UserLocal.data?['cityNameFr'] as String? ?? ''));
      ...
      final drivers = _driversList.where((d) {
        ...
        if (_selectedCity != null && _selectedCity!.isNotEmpty) {
          ...
          cityMatch = dAr == sel || dFr == sel || dCn == sel;
        } else {
          if (uCityAr.isNotEmpty) cityMatch = dAr == uCityAr || dFr == uCityAr || dCn == uCityAr;
          if (!cityMatch && uCityFr.isNotEmpty) cityMatch = dAr == uCityFr || dFr == uCityFr || dCn == uCityFr;
        }
  ...
  class _MapPickerScreenState extends State<MapPickerScreen> {
    ...
    Future<void> _reverseGeocode(LatLng ll) async {
      ...
      try {
        // الطلبان بالتوازي (كانا متسلسلين = ضعف الانتظار) مع مهلة قصيرة
        ...
        final results = await Future.wait([
          http.get(Uri.parse(urlAr), headers: const {'User-Agent': 'walyyid-app'}),
          http.get(Uri.parse(urlFr), headers: const {'User-Agent': 'walyyid-app'}),
        ]).timeout(const Duration(seconds: 5));
        final respAr = results[0];
        final respFr = results[1];
        ...
        // فشل جلب الاسم — نُبقي الحقول كما هي دون تعطيل المستخدم
      } finally {
