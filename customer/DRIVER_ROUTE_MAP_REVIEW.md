# مراجعة خريطة مسار التوصيل — ملفات السائق

## الملفات المشمولة

| الملف | الدور |
|---|---|
| `driver_route_map_screen.dart` | الشاشة الرئيسية لخريطة المسار (Google Maps + polylines + markers) |
| `driver_active_orders.dart` | شاشة الطلبات النشطة — فيها الزر اللي كيفتح خريطة المسار |
| `driver_order_detail_screen.dart` | تفاصيل الطلبية — فيها أزرار فتح الخرائط |

---

## الملف الرئيسي: driver_route_map_screen.dart
...
class _DriverRouteMapScreenState extends State<DriverRouteMapScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      Future.delayed(const Duration(seconds: 15), () {
        if (mounted && _fullSequence.isEmpty && _currentLatLng != null) {
          debugPrint('⏰ 15s timeout — نحسب المسار من موقع السائق');
          _forceRedrawRoute();
        }
      });
    });
  ...
  List<LatLng> _getRemainingSequence() {
    ...
    for (int j = bestIdx; j < _fullSequence.length; j++) {
      if (_fullSequence[j] == points.last) continue;
      points.add(_fullSequence[j]);
    }
    return points;
  }
  ...
  void _optimizeFullRoute(LatLng start, Map<String, Set<_StoreKey>> reqStores) {
    ...
    bool canReverse(int i, int j) {
      final Set<String> custsInSeg = {};
      final Set<_StoreKey> storesInSeg = {};
      for (int k = i; k <= j; k++) {
        if (custName[k] != null) {
          custsInSeg.add(custName[k]!);
        } else if (isStore[k]) {
          for (final key in _storeNames.keys) {
            if (_samePoint(_fullSequence[k], LatLng(key.lat, key.lng))) {
              storesInSeg.add(key);
              break;
            }
          }
      ...
      for (final name in custsInSeg) {
        final required = reqStores[name] ?? {};
        for (final req in required) {
          if (storesInSeg.contains(req)) return false;
        }
      }
      return true;
    }
  ...
  Future<void> _buildCustomMarkers() async {
    ...
    for (final key in _storeNames.keys) {
      ...
      final icon = await _createStoreMarker(
        label: isFinished ? "✓" : activeItems.toString(),
        storeName: name,
        index: storeIndex,
    ...
    onTap: () {
      ...
      _selectStore(key, LatLng(key.lat, key.lng));
    }));
      storeIndex++;
    }
    ...
    for (final cust in _customerData) {
      ...
      markers.add(Marker(
        ...
        icon: await _createCustomerMarker(custName, isComplete: complete),
        anchor: const Offset(0.5, 1.0),
        infoWindow: InfoWindow(title: 'زبون: $custName${complete ? ' ✓' : ''}')));
    }
  ...
  Widget _buildLegend() {
    ...
    for (int i = 1; i < _fullSequence.length; i++) {
      ...
      for (final key in _storeNames.keys) {
        if (_samePoint(pos, LatLng(key.lat, key.lng))) {
          matched = _storeNames[key];
          break;
        }
      }
      if (matched != null) {
        stopNames.add(matched);
        continue;
      }
      for (final cust in _customerData) {
        final custPos = cust['latlng'] as LatLng;
        if (_samePoint(pos, custPos)) {
          stopNames.add(cust['name'] ?? 'زبون');
          break;
        }
      }
    }
            ...
            child: Column(
              ...
              children: [
                const Text(
                  ...
                  style: TextStyle(color: Colors.white70, fontSize: 11, fontWeight: FontWeight.bold, fontFamily: 'Amiri')),
                              ...
                              children: [
                                Expanded(
                                  child: Text(
                                    stopNames[i],
                                    textAlign: TextAlign.right,
                                    style: const TextStyle(color: Colors.white, fontSize: 10, fontFamily: 'Amiri', fontWeight: FontWeight.w600))),
                                const SizedBox(width: 4),
                                const Text("إلى :", style: TextStyle(color: Colors.white70, fontSize: 9, fontFamily: 'Amiri')),
                                const SizedBox(width: 6),
                                Container(
                                  width: 12, height: 4,
                                  decoration: BoxDecoration(
...
### driver_active_orders.dart — الزر اللي كيفتح الخريطة
...
### driver_order_detail_screen.dart — أزرار فتح الخرائط

```dart
// فتح خريطة الزبون (Google Maps)
...
### 1. الخط يبدا من المحل بدل السائق
**المشكلة:** `_forceRedrawRoute()` و `_getRoadDirection()` و `_getRemainingSequence()` كانوا يستعملون `_firstStoreOrFallback()` كـ fallback كي يكون `_currentLatLng` null.

**الحل:** الآن `_forceRedrawRoute()` كيحصل على GPS أولاً، وإذا ما قدرش → يرسم المسار بين المحلات فقط كـ fallback.

### 2. السائق واقف → الخط ما كيرسمش (المشكلة الأساسية)
**المشكلة:** الكود كان كيعتمد كلياً على `_positionStream` (بث الموقع). إذا كان السائق واقفاً عند فتح الصفحة، الـ Stream ما كيرسلشبيانات فوراً، فيبقى `_currentLatLng` فاضي والخط ما كيرسمش.

**الحل:** أضفنا `_initializeFirstLocation()` في `initState` كtttt تطلب GPS فور فتح الصفحة بـ `Geolocator.getCurrentPosition()` بدل الانتظار. الآن:
1. الصفحة تتفتح → `getCurrentPosition()` يجلب الموقع فوراً
2. `_currentLatLng` يتفضل → `_getRoadDirection()` يحسب ويرسم الخط
3. `_startTracking()` يبدا البث للمتابعة

### 3. أيقونة مكررة في الهيدر
**المشكلة:** كان كاين `_HeaderIconButton` (تفاعلي) + `Container` (ثابت) كلاهما يستعمل `Icons.route_rounded`.

**الحل:** شلنا الـ Container الاستاتيكية.

### 3. Timeout كان يحسب بدون GPS
**المشكلة:** الـ 10s timeout كان كيعمل `_forceRedrawRoute()` حتى لو GPS ما كانش متاح.

**الحل:** تغير لـ 15s + يتأكد أن `_currentLatLng` موجود.

---

## ملاحظات للمراجعة

1. **الخطوط مستقيمة بدل مسارات طريق حقيقية:** الـ polylines كترسم خطوط مستقيمة بين النقاط بدل Google Directions API. يمكن تحسينها باستعمال `google_maps_webservice` أو ربط مباشر بـ Directions API.

2. **_GPS قد يأخذ وقت:** الأول fix قد يأخذ بضع ثواني. الـ `myLocationEnabled: true` في الخريطة كيعرض نقطة زرقاء للسائق، لكن الـ polyline ما يبداش حتى يوصل GPS.

3. **الخوارزمية (Nearest Neighbor + 2-opt):** كوالية للتوصيل المتعدد (Multi-Stop Delivery). المفروض تحسنت عبر مقارنة المسافات الحقيقية (Haversine) بدل المسافة المستقيمة.
